...
const dict = {
  en: {
    ...
    pm2: {
      ...
      confirmRestart: 'Restart the PM2 process? (will stop gctoac if it holds the port)',
      ...
      restarts: 'Restarts',
      portBusy: 'Port busy',
      exclusive: 'Only one runner: either PM2 or gctoac start — not both. Start via PM2 will stop gctoac on the same port.',
      hint: 'Controls only "grok-openai-gateway". Do not run gctoac start while PM2 is online.',
  ...
  'zh-Hant': {
    ...
    loginLostKey: '舊 key 無法找回（只存 hash），請 create 新 admin key。',
    loginCopy: '複製',
    loginCopied: '已複製',
    needKey: '請輸入 API key',
    logout: '登出',
    ...
    chat: {
      ...
      keyMode: 'API key',
      useSessionKey: '目前登入 admin key',
      useCustomKey: '自訂 key',
      customKey: 'Key',
      includeReasoning: '顯示思考',
      reasoning: '思考過程',
      needKey: '請輸入或選擇 API key',
    },
    status: {
      success: '成功',
      error: '錯誤',
    ...
    pm2: {
      ...
      confirmRestart: '確定重啟 PM2？（若 gctoac 佔住 port 會先停 gctoac）',
      unavailable: 'PM2 不可用',
      ...
      restarts: '重啟次數',
      portBusy: 'Port 被佔',
      exclusive: '只能二選一：PM2 或 gctoac start，唔好同時開。PM2 啟動會先釋放 port（停 gctoac）。',
      hint: '只控制「grok-openai-gateway」。PM2 online 時唔好再 gctoac start。',
