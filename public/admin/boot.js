...
… (${e("media.previewTruncated")})`:f;v.innerHTML=`<pre class="media-lb-text">${t(w)}</pre>`}).catch(()=>{const f=document.querySelector("#modal-back .media-lb-stage");f&&(f.innerHTML=`<div class="error-box">${t(e("media.previewFail"))}</div>`)})}function La(){return`
  ...
  `}function ie(a,s){return`<button type="button" class="nav-btn ${c.page===a?"active":""}" data-nav="${a}">${t(s)}</button>`}function oe(){ft(),document.querySelectorAll("[data-nav]").forEach(s=>{s.onclick=()=>{ft(),sa(s.dataset.nav)}});const a=()=>Et(!0);document.getElementById("btn-logout")?.addEventListener("click",a),document.getElementById("btn-logout-mobile")?.addEventListener("click",a),document.getElementById("nav-open")?.addEventListener("click",bs),document.getElementById("nav-backdrop")?.addEventListener("click",ft),document.addEventListener("keydown",s=>{s.key==="Escape"&&ft()},{once:!0}),document.querySelectorAll("[data-lang]").forEach(s=>{s.onclick=()=>{Ma(s.dataset.lang),Lt().catch($)}})}function $(a){console.error(a),F(a.message||String(a))}async function Ha(){if(!c.key)return!1;const a=await S("/me");return c.me=a.data,!0}async function dt(a=!1){try{const s=await S(`/models${a?"?refresh=1":""}`);return c.models=s.data?.models||[],s.data}catch{return c.models=[],{models:[],source:"fallback",defaultModel:""}}}async function ct(){try{const a=await S("/keys?all=1");c.keys=a.data||[]}catch{c.keys=[]}}function we(a){const s=(Array.isArray(a)?a:[a]).filter(Boolean);return s.length?`<div class="page-meta" role="status">${s.map(o=>`<span>${typeof o=="string"?t(o):o}</span>`).join('<span class="page-meta-sep" aria-hidden="true">·</span>')}</div>`:""}function He({title:a,hint:s,meta:n,searchHtml:o,gridHtml:i}){return`
    ...
    </div>`}function Ze(a,s,n){document.getElementById(`${a}-prev`)?.addEventListener("click",()=>{s.offset=Math.max(0,s.offset-s.limit),n()}),document.getElementById(`${a}-next`)?.addEventListener("click",()=>{s.offset=s.offset+s.limit,n()}),document.getElementById(`${a}-limit`)?.addEventListener("change",o=>{s.limit=Number(o.target.value)||20,s.offset=0,n()})}function ue(){document.querySelectorAll("#modal-back video, #modal-back audio").forEach(a=>{try{a.pause()}catch{}}),Ca(),document.getElementById("modal-back")?.remove(),c.modal=null}function et({title:a,subtitle:s,bodyHtml:n,footerHtml:o,size:i="md"}){ue();const r=`
    ...
    </div>`;document.getElementById("app").insertAdjacentHTML("beforeend",r);const d=()=>ue();document.getElementById("modal-close").onclick=d,document.getElementById("modal-back").onclick=m=>{m.target.id==="modal-back"&&d()};const u=m=>{m.key==="Escape"&&(d(),document.removeEventListener("keydown",u))};document.addEventListener("keydown",u)}async function Da(){const a="gctoac admin otp";document.getElementById("app").innerHTML=`
  ...
  `,document.querySelectorAll("[data-lang]").forEach(s=>{s.onclick=()=>{Ma(s.dataset.lang),Da().catch($)}}),document.getElementById("btn-copy-cmd").onclick=async()=>{try{await navigator.clipboard.writeText(a);const s=document.getElementById("btn-copy-cmd");s.textContent=e("loginCopied"),setTimeout(()=>{s.textContent=e("loginCopy")},1500)}catch{}},document.getElementById("btn-login").onclick=async()=>{const s=document.getElementById("login-key").value.trim();if(!s)return F(e("needOtp"));try{const n=await fetch("/admin/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:s})}),o=await n.json().catch(()=>({}));if(!n.ok)throw new Error(o?.error?.message||o?.message||e("loginOtpFail"));const i=o?.data?.token;if(!i)throw new Error(e("loginOtpFail"));c.key=i,sessionStorage.setItem(Pt,i),await Ha(),c.error="",sa("dashboard")}catch(n){c.key="",sessionStorage.removeItem(Pt),F(n.message||e("loginOtpFail"))}},document.getElementById("login-key").onkeydown=s=>{s.key==="Enter"&&document.getElementById("btn-login").click()}}function ge({label:a,value:s,sub:n,tone:o,href:i,valueId:r,subId:d}){const u=o?` dash-kpi--${o}`:"",m=r?` id="${t(r)}"`:"",p=d?` id="${t(d)}"`:"",l=`
    ...
    <tr>
      ...
      <td>${ee(g.createdAt)}</td>
    </tr>`).join(""),A=be({headHtml:`
      <th>${t(e("chats.request"))}</th>
      <th>${t(e("chats.model"))}</th>
      <th>${t(e("chats.status"))}</th>
      <th>${t(e("chats.mode"))}</th>
      <th>${t(e("chats.duration"))}</th>
      <th>${t(e("chats.time"))}</th>`,bodyHtml:B,colSpan:6,emptyText:e("dash.empty")}),M=Math.max(1,...m.map(g=>g.requests||0)),H=m.length?m.map(g=>{const k=Math.round((g.requests||0)/M*100);return`
          ...
          </div>`}).join(""):`<div class="data-empty" style="padding:20px"><strong>${t(e("dash.emptyModels"))}</strong></div>`,G=(g,k)=>`<span class="dash-rule-chip ${g?"is-on":"is-off"}">${t(k)}</span>`,V=d?`
    ...
    <div class="dash-kpi-grid">
      ${ge({label:e("dash.kpi24h"),value:n.chats24h??0,sub:q("dash.kpi24hSub",{ok:n.success24h??0,err:n.error24h??0}),tone:"primary",href:"chats"})}
      ${ge({label:e("dash.kpiSuccessRate"),value:`${p}%`,sub:q("dash.kpiSuccessRateSub",{all:l}),tone:p>=90?"ok":p>=70?"warn":"danger",href:"usage"})}
      ${ge({label:e("dash.kpiErrors"),value:n.error24h??0,sub:q("dash.kpiErrorsSub",{all:n.errors??0}),tone:(n.error24h||0)>0?"warn":"ok",href:"chats"})}
      ${ge({label:e("dash.kpiQueue"),value:h,sub:f,tone:v,href:"queue"})}
      ${ge({label:e("dash.kpiSafe"),value:w,sub:L,tone:u?y?"ok":"warn":"",href:"settings"})}
      ...
      ${ge({label:e("dash.kpiMedia")||"Media",value:n.mediaAssets??0,sub:q("dash.kpiMediaSub",{n:n.mediaAssets24h??0}),href:"media"})}
      ${ge({label:e("dash.kpiConv"),value:n.conversations??0,sub:q("dash.kpiConvSub",{n:n.conversations24h??0}),href:"chat"})}
...
`):s,hasRoles:!0,blocks:r}}async function vt(){await Promise.all([dt(),ct()]);const a=c.chatFilter,s=new URLSearchParams;if(s.set("limit",String(a.limit)),s.set("offset",String(a.offset)),a.status&&s.set("status",a.status),a.model&&s.set("model",a.model),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.q&&s.set("q",a.q),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const l=new Date(a.to);l.setHours(23,59,59,999),s.set("to",l.toISOString())}a.policyMode&&s.set("policyMode",a.policyMode),a.hasDocuments!==""&&s.set("hasDocuments",a.hasDocuments);const n=await S(`/chats?${s}`),o=n.total||0,i=[`<option value="">${t(e("chats.allModels"))}</option>`,...c.models.map(l=>`<option value="${t(l)}" ${a.model===l?"selected":""}>${t(l)}</option>`)].join(""),r=[`<option value="">${t(e("chats.allKeys"))}</option>`,...c.keys.map(l=>`<option value="${l.id}" ${a.apiKeyId===l.id?"selected":""}>${t(l.name)} (${t(l.keyPrefix)})</option>`)].join(""),d=(n.items||[]).map(l=>{const b=Oa(l.promptPreview||""),h=!!b.system,f=h?b.body.slice(0,160):l.promptPreview||"";return`
    <tr>
      ...
      <td>${ee(l.createdAt)}</td>
    </tr>`}).join(""),u=He({title:e("chats.filterTitle")||e("common.filterTitle"),hint:e("chats.filterHint")||e("common.filterHint"),meta:q("common.pagerTotal",{n:o}),searchHtml:`
  ...
  `),oe(),Ze("chats",c.chatFilter,()=>vt().catch($));const p=()=>{c.chatFilter.q=document.getElementById("f-q").value.trim(),c.chatFilter.status=document.getElementById("f-status").value,c.chatFilter.model=document.getElementById("f-model").value,c.chatFilter.apiKeyId=document.getElementById("f-key").value,c.chatFilter.policyMode=document.getElementById("f-mode").value,c.chatFilter.from=document.getElementById("f-from").value,c.chatFilter.to=document.getElementById("f-to").value,c.chatFilter.hasDocuments=document.getElementById("f-docs").checked?"true":"",c.chatFilter.offset=0,vt().catch($)};document.querySelector("[data-filter-apply]").onclick=p,document.getElementById("f-q").onkeydown=l=>{l.key==="Enter"&&p()},document.querySelector("[data-filter-reset]").onclick=()=>{c.chatFilter={q:"",status:"",model:"",apiKeyId:"",from:"",to:"",policyMode:"",hasDocuments:"",limit:50,offset:0},vt().catch($)},document.querySelectorAll("[data-chat]").forEach(l=>{l.onclick=()=>Fa(l.dataset.chat)})}async function Fa(a){const{data:s}=await S(`/chats/${a}`),n=s.response||{},o=s.documents||[];let i=`<p class="muted">${t(e("chats.noAttach"))}</p>`;if(o.length){const m=[];for(const p of o){let l="";if(lt(p.mimeType))try{const b=await S(`/documents/${p.id}`),h=await Na(b.data||{id:p.id,isImage:!0,mimeType:p.mimeType});h?.src&&(l=`<img class="preview" src="${h.src}" alt="${t(p.originalName)}" />`)}catch{l=`<span class="muted">${t(e("chats.previewFailed"))}</span>`}m.push(`
    ...
    <div class="modal-meta-foot muted">${t(e("common.ipLabel"))}: ${t(s.ip||"—")} · ${t(e("common.uaLabel"))}: ${t(s.userAgent||"—")} · ${ee(s.createdAt)}</div>`;et({title:e("chats.detail"),subtitle:`${t(s.requestId)} · ${da(s.status)} ${Ct(s.policyMode||"-")}`,bodyHtml:u,size:"xl",footerHtml:`<button type="button" class="btn secondary sm" id="modal-ok">${t(e("chats.close"))}</button>`}),document.getElementById("modal-ok")?.addEventListener("click",()=>ue()),document.querySelector('[data-copy="system"]')?.addEventListener("click",()=>{navigator.clipboard.writeText(r.system||"")}),document.querySelector('[data-copy="prompt"]')?.addEventListener("click",()=>{navigator.clipboard.writeText(r.body||s.prompt||"")}),document.querySelector('[data-copy="raw-prompt"]')?.addEventListener("click",()=>{navigator.clipboard.writeText(s.prompt||"")}),document.querySelector('[data-copy="content"]')?.addEventListener("click",()=>{navigator.clipboard.writeText(n.content||"")}),document.querySelectorAll("[data-open-doc]").forEach(m=>{m.onclick=()=>Ka(m.dataset.openDoc)})}async function Ke(){const a=c.keyFilter;let s={};try{const p=await S("/usage");for(const l of p.data?.perKey||[])s[l.apiKeyId]=l}catch{}const n=new URLSearchParams;n.set("limit",String(a.limit)),n.set("offset",String(a.offset)),a.q&&n.set("q",a.q),a.role&&n.set("role",a.role),a.mode&&n.set("mode",a.mode),a.isActive!==""&&n.set("isActive",a.isActive);const o=await S(`/keys?${n}`),i=o.data||[],r=o.total??i.length,d=i.map(p=>{const l=s[p.id],b=l?.requests??"—",h=l?Math.round((l.utilization||0)*100):0,f=p.ipWhitelist||[],v=f.length?q("keys.ipCount",{n:f.length}):e("keys.ipAll");return`
    <tr>
    ...
    </tr>`}).join(""),u=He({title:e("common.filterTitle"),hint:e("common.filterHint"),meta:q("common.pagerTotal",{n:r}),searchHtml:`
      <div class="data-filter-search">
...
`);et({title:e(s?"keys.edit":"keys.new"),subtitle:s?`${t(a?.name||"")} · ${t(a?.keyPrefix||"")}…`:"",size:"md",bodyHtml:`
      ...
      <button type="button" class="btn sm" id="k-save">${t(e("common.save"))}</button>`}),document.getElementById("k-role").value=a?.role||"client",document.getElementById("k-mode").value=a?.mode||"safe",s&&(document.getElementById("k-active").value=String(a.isActive)),document.getElementById("k-cancel").onclick=()=>ue(),document.getElementById("k-save").onclick=async()=>{const o=document.getElementById("k-ip").value.split(/[\n,]+/).map(r=>r.trim()).filter(Boolean),i={name:document.getElementById("k-name").value.trim(),role:document.getElementById("k-role").value,mode:document.getElementById("k-mode").value,rateLimit:Number(document.getElementById("k-rate").value||60),maxTurns:document.getElementById("k-turns").value?Number(document.getElementById("k-turns").value):null,timeoutMs:document.getElementById("k-timeout").value?Number(document.getElementById("k-timeout").value):null,ipWhitelist:o};try{if(s)i.isActive=document.getElementById("k-active").value==="true",await S(`/keys/${a.id}`,{method:"PATCH",body:JSON.stringify(i)}),ue(),Ke().catch($);else{const r=await S("/keys",{method:"POST",body:JSON.stringify(i)}),d=document.getElementById("k-created");d&&(d.hidden=!1,d.textContent=`${e("keys.keyOnce")}
${r.data?.key||JSON.stringify(r.data)}`);const u=document.getElementById("k-save");u&&(u.textContent=e("chats.close"),u.onclick=()=>{ue(),Ke().catch($)})}}catch(r){$(r)}}}function _a(a){return e(a==="filesystem"?"docs.storageFs":"docs.storageDb")}async function Ra(a,s){try{const n=await fetch(`${Zt}/documents/${a}/download`,{headers:c.key?{Authorization:`Bearer ${c.key}`}:{}});if(!n.ok){const d=await n.text();let u=d;try{u=JSON.parse(d).error?.message||d}catch{}throw new Error(u||e("docs.downloadFail"))}const o=await n.blob(),i=URL.createObjectURL(o),r=document.createElement("a");r.href=i,r.download=s||"download",document.body.appendChild(r),r.click(),r.remove(),URL.revokeObjectURL(i)}catch(n){F(n.message||e("docs.downloadFail"))}}async function st(){await ct();const a=c.docFilter,s=new URLSearchParams({limit:String(a.limit),offset:String(a.offset)});if(a.q&&s.set("q",a.q),a.apiKeyId&&s.set("apiKeyId",a.apiKeyId),a.storageType&&s.set("storageType",a.storageType),a.from&&s.set("from",new Date(a.from).toISOString()),a.to){const l=new Date(a.to);l.setHours(23,59,59,999),s.set("to",l.toISOString())}const n=await S(`/documents?${s}`),o=n.total??0,i=n.meta||{},r=q("docs.storageHint",{dir:i.storageDir||"—",dbMax:$e(i.documentDbMaxBytes),upMax:$e(i.uploadMaxBytes)}),d=[`<option value="">${t(e("common.all"))}</option>`,...c.keys.map(l=>`<option value="${l.id}" ${a.apiKeyId===l.id?"selected":""}>${t(l.name)}</option>`)].join(""),u=(n.data||[]).map(l=>`
    <tr>
    ...
    </tr>`).join(""),m=He({title:e("common.filterTitle"),hint:e("common.filterHint"),meta:q("common.pagerTotal",{n:o}),searchHtml:`
      <div class="data-filter-search">
  ...
  `),oe(),document.getElementById("s-tools").value=a.safeToolsMode||"none";const r=()=>Ve();["s-tools","s-turns","s-timeout"].forEach(d=>{const u=document.getElementById(d);u&&(u.addEventListener("change",r),u.addEventListener("input",r))}),Ve(),document.getElementById("s-master-global")?.addEventListener("click",async()=>{const d=!Ue("s-master-global");We("s-master-global",d,e("settings.masterOn"),e("settings.masterOff")),Qe("settings-root",!d),Ge("settings-disabled-banner",!d),Ve();try{await S("/settings",{method:"PUT",body:JSON.stringify({globalSafeMode:d,safeToolsMode:document.getElementById("s-tools").value,safeMaxTurns:Number(document.getElementById("s-turns").value),safeTimeoutMs:Number(document.getElementById("s-timeout").value),defaultModel:document.getElementById("s-model").value.trim()})})}catch(u){We("s-master-global",!d,e("settings.masterOn"),e("settings.masterOff")),Qe("settings-root",d),Ge("settings-disabled-banner",d),$(u)}}),document.getElementById("btn-refresh-models").onclick=async()=>{await dt(!0),ja().catch($)},document.getElementById("s-save").onclick=async()=>{try{await S("/settings",{method:"PUT",body:JSON.stringify({globalSafeMode:Ue("s-master-global"),safeToolsMode:document.getElementById("s-tools").value,safeMaxTurns:Number(document.getElementById("s-turns").value),safeTimeoutMs:Number(document.getElementById("s-timeout").value),defaultModel:document.getElementById("s-model").value.trim()})}),Ve();const d=document.querySelector("#flash-error");d&&(d.hidden=!1,d.classList.add("flash-ok"),d.textContent=e("settings.saved"),setTimeout(()=>{d.hidden=!0,d.classList.remove("flash-ok"),d.textContent=""},2e3))}catch(d){$(d)}},document.querySelectorAll("[data-apply-preset]").forEach(d=>{d.addEventListener("click",async()=>{if(d.disabled)return;const u=d.getAttribute("data-apply-preset"),m=Wt().find(p=>p.id===u);m&&await xs(m)})}),document.getElementById("s-disable-panel").onclick=async()=>{if(await X({message:e("settings.disablePanelConfirm"),variant:"danger",confirmText:e("settings.disablePanel")}))try{await S("/settings",{method:"PUT",body:JSON.stringify({adminPanelEnabled:!1})}),await ce({message:e("settings.disablePanelDone"),title:e("common.notice")}),Et(!1)}catch(d){$(d)}}}async function kt(){const a=await S("/api-features");if(c.page!=="apiFeatures")return;const s=a.data||{},n=[{id:"protocols",title:e("apiFeatures.groupProtocols"),tabLabel:e("apiFeatures.tabProtocols"),keys:["openaiChat","openaiResponses","anthropicMessages"]},{id:"media",title:e("apiFeatures.groupMedia"),tabLabel:e("apiFeatures.tabMedia"),keys:["imagesApi","filesOpenAiAlias","videoApi","audioApi"]},{id:"caps",title:e("apiFeatures.groupCaps"),tabLabel:e("apiFeatures.tabCaps"),keys:["tools","structuredOutput","vision","reasoningEffort","webSearch","subagents","planMode","memory","sessionResume","bestOfN","checkLoop","systemOverride","rules","permissionMode","sandbox"]},{id:"emu",title:e("apiFeatures.groupEmu"),tabLabel:e("apiFeatures.tabEmu"),keys:["usageEstimate","assistantsEmulation","strictSampling","forceDisableToolsInSafe"]}],o=c.apiFeaturesTab==="media"||c.apiFeaturesTab==="caps"||c.apiFeaturesTab==="emu"||c.apiFeaturesTab==="protocols"?c.apiFeaturesTab:"protocols";c.apiFeaturesTab=o;const i=f=>e(`apiFeatures.flag.${f}`)||f,r=f=>e(`apiFeatures.hint.${f}`)||"",d=f=>f.filter(v=>!!s[v]).length,u=f=>f.map(v=>{const y=!!s[v];return`
  ...
  `),oe(),document.querySelectorAll("[data-feat-tab]").forEach(f=>{f.addEventListener("click",()=>{const v=f.getAttribute("data-feat-tab")||"protocols",y=v==="media"||v==="caps"||v==="emu"||v==="protocols"?v:"protocols";c.apiFeaturesTab!==y&&(c.apiFeaturesTab=y,kt().catch($))})}),document.querySelectorAll("[data-feat-toggle]").forEach(f=>{f.addEventListener("click",async()=>{const v=f.getAttribute("data-feat-toggle");if(!v)return;const y=!f.classList.contains("is-on");try{await S("/api-features",{method:"PUT",body:JSON.stringify({[v]:y})}),await kt()}catch(w){$(w)}})}),document.querySelectorAll("[data-feat-preset]").forEach(f=>{f.addEventListener("click",async()=>{const v=f.getAttribute("data-feat-preset");if(await X({message:q("apiFeatures.presetConfirm",{name:v}),confirmText:e("common.confirm")}))try{await S("/api-features/preset",{method:"POST",body:JSON.stringify({name:v})}),await kt()}catch(y){$(y)}})})}async function Pe(){c.mediaFilter||(c.mediaFilter={tab:"studio",q:"",kind:"",provider:"",from:"",to:"",limit:20,offset:0}),c.mediaFilter.tab||(c.mediaFilter.tab="studio");const a=c.mediaFilter,s=a.tab==="assets"||a.tab==="jobs"||a.tab==="studio"?a.tab:"studio";a.tab=s;const n=new URLSearchParams({limit:String(a.limit),offset:String(a.offset)});if(a.q&&n.set("q",a.q),a.kind&&n.set("kind",a.kind),a.provider&&n.set("provider",a.provider),a.from&&n.set("from",new Date(a.from).toISOString()),a.to){const g=new Date(a.to);g.setHours(23,59,59,999),n.set("to",g.toISOString())}const[o,,i,r]=await Promise.all([dt(!1).catch(()=>({models:c.models||[],defaultModel:""})),ct().catch(()=>{}),S(`/media/assets?${n}`),S("/media/jobs?limit=50").catch(()=>({data:[],total:0}))]),d=i.data||[],u=i.total??d.length,m=r.data||[],p=r.total??m.length,l=(c.keys||[]).filter(g=>g.isActive!==!1&&(g.mode==="agent"||g.role==="admin")),b=[`<option value="">${t(e("media.generateKeySession"))}</option>`,...l.map(g=>`<option value="${t(g.id)}">${t(g.name||g.id)} · ${t(g.keyPrefix||"")}… · ${t(g.mode||"")}</option>`)].join(""),h=o.models?.length?o.models:c.models||[],f=o.defaultModel||h[0]||"",v=h.length?h.map(g=>`<option value="${t(g)}" ${g===f?"selected":""}>${t(g)}${g===f?` · ${t(e("media.modelDefault"))}`:""}</option>`).join(""):`<option value="">${t(f||e("media.modelEmpty"))}</option>`,y=[["1:1","1:1 · square"],["16:9","16:9 · landscape"],["9:16","9:16 · portrait / story"],["4:3","4:3"],["3:4","3:4"],["3:2","3:2"],["2:3","2:3"],["auto","auto"]].map(([g,k],x)=>`<option value="${g}" ${x===0?"selected":""}>${t(k)}</option>`).join(""),w=d.map(g=>{const k=g.mime||"",x=g.filename||g.originalName||"",_=fs(k,x),R=ra(k,x)||"",P=_?`<button type="button" class="btn ghost sm" data-media-preview="${t(g.id)}" data-media-mime="${t(k)}" data-media-name="${t(x)}" data-media-kind="${t(g.kind||"")}" data-media-bytes="${t(String(g.bytes??""))}" data-media-prompt="${t(g.prompt||"")}" data-preview-kind="${t(R)}" title="${t(e("media.preview"))}">${t(e("media.preview"))}</button>`:"";return`
    <tr>
      <td>
    ...
    </tr>`}).join(""),L=He({title:e("common.filterTitle"),hint:e("common.filterHint"),meta:q("common.pagerTotal",{n:u}),searchHtml:`
      <div class="data-filter-search">
      ...
      <th>${t(e("common.actions"))}</th>`,bodyHtml:w,colSpan:8,emptyText:e("media.empty"),pagerHtml:ke({total:u,limit:a.limit,offset:a.offset,idPrefix:"media"})}),A=m.map(g=>`
    <tr>
      <td>
        <div class="cell-primary mono" title="${t(g.id)}">${t(String(g.id).slice(0,8))}…</div>
      </td>
      ...
      <th>${t(e("media.prompt"))}</th>
      <th>Asset</th>
      <th>${t(e("media.created"))}</th>`,bodyHtml:A,colSpan:5,emptyText:e("media.jobsEmpty")}),H=`
  ...
  `),oe(),document.querySelectorAll("[data-media-tab]").forEach(g=>{g.addEventListener("click",()=>{const k=g.getAttribute("data-media-tab")||"studio",x=k==="assets"||k==="jobs"||k==="studio"?k:"studio";c.mediaFilter.tab!==x&&(c.mediaFilter.tab=x,Pe().catch($))})});async function V(g){const k=await fetch(`/admin/api/media/assets/${g}/download`,{headers:{Authorization:`Bearer ${c.key}`}});if(!k.ok)throw new Error(await k.text());return k.blob()}{let g="generate",k=null;const x=()=>{const E=document.getElementById("mg-source-chip"),D=document.getElementById("mg-clear-source");if(!E)return;if(!k){E.hidden=!0,E.innerHTML="",D&&(D.hidden=!0);return}const de=k.kind==="file"?e("media.sourceKindUpload"):k.kind==="asset"?e("media.sourceKindAsset"):e("media.sourceKindDocument");E.hidden=!1,E.innerHTML=`<span class="chip">${t(de)}</span> <span class="mono">${t(k.name||k.id||"")}</span>`,D&&(D.hidden=!1)},_=E=>{k=E;const D=document.getElementById("mg-file");D&&E?.kind!=="file"&&(D.value=""),x()},R=E=>{g=E==="edit"||E==="video"?E:"generate",document.querySelectorAll("[data-mg-mode]").forEach(re=>{const Ht=re.getAttribute("data-mg-mode")===g;re.classList.toggle("is-active",Ht),re.setAttribute("aria-selected",Ht?"true":"false")});const D=document.getElementById("mg-source-section"),de=document.getElementById("mg-n-wrap"),Me=document.getElementById("mg-duration-wrap"),Fe=document.getElementById("mg-submit");D&&(D.hidden=g==="generate"),de&&(de.hidden=g==="video"),Me&&(Me.hidden=g!=="video"),Fe&&(Fe.textContent=e(g==="edit"?"media.editSubmit":g==="video"?"media.videoSubmit":"media.generateSubmit"));const pe=document.getElementById("mg-prompt");pe&&(pe.placeholder=e(g==="edit"?"media.editPromptPh":g==="video"?"media.videoPromptPh":"media.generatePromptPh"));const te=document.getElementById("mg-drop-title"),fe=document.getElementById("mg-drop-hint");te&&(te.textContent=e(g==="video"?"media.dropTitleVideo":"media.dropTitle")),fe&&(fe.textContent=e(g==="video"?"media.dropHintVideo":"media.dropHint"))};document.querySelectorAll("[data-mg-mode]").forEach(E=>{E.addEventListener("click",()=>R(E.getAttribute("data-mg-mode")||"generate"))}),R("generate");const P=document.getElementById("mg-dropzone"),O=document.getElementById("mg-file"),K=E=>E?E.type&&E.type.startsWith("image/")?!0:/\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(E.name||""):!1,U=E=>{const D=[...E||[]].find(K);if(!D){F(e("media.sourceNeedImage"));return}_({kind:"file",file:D,name:D.name,mime:D.type||"image/*"}),F("")};if(document.getElementById("mg-pick-file")?.addEventListener("click",E=>{E.preventDefault(),E.stopPropagation(),O?.click()}),O?.addEventListener("change",()=>{O.files?.length&&U(O.files)}),document.getElementById("mg-clear-source")?.addEventListener("click",E=>{E.preventDefault(),E.stopPropagation(),_(null)}),document.getElementById("mg-pick-lib")?.addEventListener("click",E=>{E.preventDefault(),E.stopPropagation(),Is({imagesOnly:!0,onPick:D=>{_({kind:D.kind,id:D.id,name:D.name,mime:D.mime}),F("")}}).catch(D=>F(D.message||e("media.libraryLoadFail")))}),P&&(P.addEventListener("click",E=>{E.target.closest("button")||O?.click()}),P.addEventListener("keydown",E=>{(E.key==="Enter"||E.key===" ")&&(E.preventDefault(),O?.click())}),["dragenter","dragover"].forEach(E=>{P.addEventListener(E,D=>{D.preventDefault(),D.stopPropagation(),P.classList.add("is-dragover")})}),["dragleave","drop"].forEach(E=>{P.addEventListener(E,D=>{D.preventDefault(),D.stopPropagation(),P.classList.remove("is-dragover")})}),P.addEventListener("drop",E=>{const D=E.dataTransfer;D?.files?.length&&U(D.files)})),c._mediaDragAbort)try{c._mediaDragAbort.abort()}catch{}c._mediaDragAbort=new AbortController;const I={signal:c._mediaDragAbort.signal},Q=document.getElementById("app");let j=0;window.addEventListener("dragenter",E=>{g!=="generate"&&[...E.dataTransfer?.types||[]].includes("Files")&&(j+=1,Q?.classList.add("is-media-file-drag"))},I),window.addEventListener("dragleave",()=>{j=Math.max(0,j-1),j===0&&Q?.classList.remove("is-media-file-drag")},I),window.addEventListener("drop",E=>{j=0,Q?.classList.remove("is-media-file-drag"),g!=="generate"&&E.dataTransfer?.files?.length&&(E.preventDefault(),U(E.dataTransfer.files))},I),window.addEventListener("dragover",E=>{g!=="generate"&&[...E.dataTransfer?.types||[]].includes("Files")&&E.preventDefault()},I),document.getElementById("mg-submit")?.addEventListener("click",async()=>{const E=document.getElementById("mg-prompt")?.value?.trim()||"";if(!E){F(e("media.generateNeedPrompt"));return}const D=document.getElementById("mg-key")?.value||"",de=document.getElementById("mg-model")?.value||void 0,Me=document.getElementById("mg-aspect")?.value||"1:1",Fe=Math.min(4,Math.max(1,Number(document.getElementById("mg-n")?.value)||1)),pe=document.getElementById("mg-submit"),te=document.getElementById("mg-status"),fe=e(g==="video"?"media.videoBusy":g==="edit"?"media.editBusy":"media.generateBusy");pe&&(pe.disabled=!0,pe.textContent=fe),te&&(te.hidden=!1,te.textContent=fe),F("");try{if(g==="edit"){if(!k)throw new Error(e("media.editNeedImage"));const J=new FormData;J.append("prompt",E),J.append("aspect_ratio",Me),J.append("n",String(Fe)),J.append("response_format","url"),de&&J.append("model",de),D&&J.append("apiKeyId",D),k.kind==="file"&&k.file?J.append("image",k.file):k.kind==="asset"&&k.id?J.append("sourceAssetId",k.id):k.kind==="document"&&k.id&&J.append("sourceDocumentId",k.id),await pa(await fetch("/admin/api/media/edit",{method:"POST",headers:{Authorization:`Bearer ${c.key}`},body:J})),te&&(te.textContent=e("media.editOk")),c.mediaFilter.tab="assets",c.mediaFilter.offset=0,await Pe();return}if(g==="video"){const J=new FormData;J.append("prompt",E),J.append("aspect_ratio",Me),J.append("seconds",String(document.getElementById("mg-duration")?.value||6)),de&&J.append("model",de),D&&J.append("apiKeyId",D),k?.kind==="file"&&k.file?J.append("image",k.file):k?.kind==="asset"&&k.id?J.append("source_asset_id",k.id):k?.kind==="document"&&k.id&&J.append("source_document_id",k.id),await pa(await fetch("/admin/api/media/videos",{method:"POST",headers:{Authorization:`Bearer ${c.key}`},body:J})),te&&(te.textContent=e("media.videoOk")),c.mediaFilter.tab="jobs",await Pe();return}const re={prompt:E,aspect_ratio:Me,n:Fe,response_format:"url"};de&&(re.model=de),D&&(re.apiKeyId=D);const pt=(await S("/media/generate",{method:"POST",body:JSON.stringify(re)}))?.data?.grok?.asset_ids||[];if(te&&(te.textContent=e("media.generateOk")),c.mediaFilter.tab="assets",c.mediaFilter.offset=0,await Pe(),pt[0])try{const J=await V(pt[0]);fa({id:pt[0],mime:J.type||"image/png",filename:`generated-${String(pt[0]).slice(0,8)}`,kind:"image",bytes:J.size,prompt:E},J)}catch{}}catch(re){$(re),te&&(te.textContent=re.message||e("media.generateFail")),pe&&(pe.disabled=!1,R(g))}})}s==="assets"&&(Ze("media",c.mediaFilter,()=>Pe().catch($)),document.querySelector("#media-tab-assets [data-filter-apply]")?.addEventListener("click",()=>{c.mediaFilter.q=document.getElementById("mf-q")?.value.trim()||"",c.mediaFilter.kind=document.getElementById("mf-kind")?.value||"",c.mediaFilter.provider=document.getElementById("mf-provider")?.value.trim()||"",c.mediaFilter.from=document.getElementById("mf-from")?.value||"",c.mediaFilter.to=document.getElementById("mf-to")?.value||"",c.mediaFilter.offset=0,Pe().catch($)}),document.querySelector("#media-tab-assets [data-filter-reset]")?.addEventListener("click",()=>{const g=c.mediaFilter.tab;c.mediaFilter={tab:g,q:"",kind:"",provider:"",from:"",to:"",limit:20,offset:0},Pe().catch($)}),document.querySelectorAll("[data-media-preview]").forEach(g=>{g.addEventListener("click",async()=>{try{const k=g.getAttribute("data-media-preview");if(!k)return;const x=g.getAttribute("data-media-mime")||"",_=g.getAttribute("data-media-name")||"",R=g.getAttribute("data-media-kind")||"",P=g.getAttribute("data-media-bytes")||"",O=g.getAttribute("data-media-prompt")||"",K=await V(k);fa({id:k,mime:x||K.type||"",filename:_,kind:R,bytes:P?Number(P):K.size,prompt:O},K)}catch(k){$(k)}})}),document.querySelectorAll("[data-media-dl]").forEach(g=>{g.addEventListener("click",async()=>{try{const k=g.getAttribute("data-media-dl"),x=g.getAttribute("data-media-name")||"",_=await V(k),R=document.createElement("a");R.href=URL.createObjectURL(_),R.download=x||`media-${String(k).slice(0,8)}`,R.click(),setTimeout(()=>URL.revokeObjectURL(R.href),3e4)}catch(k){$(k)}})}),document.querySelectorAll("[data-media-del]").forEach(g=>{g.addEventListener("click",async()=>{const k=g.getAttribute("data-media-del");if(await X({message:e("media.deleteConfirm"),variant:"danger",confirmText:e("media.delete")}))try{await S(`/media/assets/${k}`,{method:"DELETE"}),await Pe()}catch(x){$(x)}})}))}async function Is(a){const s=a.imagesOnly!==!1;let n="documents",o=0,i=null;et({title:e("media.libraryTitle"),subtitle:t(e("media.librarySubtitle")),size:"md",bodyHtml:`
      ...
      <button type="button" class="btn sm" id="mlib-add" disabled>${t(e("media.librarySelect"))}</button>`});const r=document.getElementById("mlib-list"),d=document.getElementById("mlib-q"),u=document.getElementById("mlib-add");document.getElementById("mlib-cancel")?.addEventListener("click",()=>ue());const m=()=>{u&&(u.disabled=!i,u.textContent=i?`${e("media.librarySelect")} · ${i.name.slice(0,24)}`:e("media.librarySelect"))},p=v=>String(v||"").startsWith("image/"),l=v=>/\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(String(v||"")),b=v=>{if(r){if(!v.length){r.innerHTML=`<div class="data-empty chat-lib-empty"><strong>${t(e("media.libraryEmpty"))}</strong></div>`;return}r.innerHTML=v.map(y=>{const w=i?.id===y.id&&i?.kind===y.kind;return`
    ...
    ${we([`${e("usage.window")}: ${ee(a.from)} → ${ee(a.to)} (${q("common.minutes",{n:a.windowMinutes})})`])}
    <div class="grid">
  ...
  `),oe(),document.getElementById("btn-usage-refresh").onclick=()=>le().catch($),document.querySelectorAll("[data-usage-tab]").forEach(M=>{M.onclick=()=>{const H=M.dataset.usageTab==="key"?"key":"model";c.usageFilter.tab!==H&&(c.usageFilter.tab=H,le().catch($))}}),document.getElementById("umodel-prev")?.addEventListener("click",()=>{c.usageFilter.modelPage=Math.max(0,o.modelPage-1),le().catch($)}),document.getElementById("umodel-next")?.addEventListener("click",()=>{(o.modelPage+1)*i<d&&(c.usageFilter.modelPage+=1,le().catch($))}),document.getElementById("umodel-limit")?.addEventListener("change",M=>{c.usageFilter.pageSize=Number(M.target.value)||10,c.usageFilter.modelPage=0,le().catch($)}),document.getElementById("ukey-prev")?.addEventListener("click",()=>{c.usageFilter.keyPage=Math.max(0,o.keyPage-1),le().catch($)}),document.getElementById("ukey-next")?.addEventListener("click",()=>{(o.keyPage+1)*i<l&&(c.usageFilter.keyPage+=1,le().catch($))}),document.getElementById("ukey-limit")?.addEventListener("change",M=>{c.usageFilter.pageSize=Number(M.target.value)||10,c.usageFilter.keyPage=0,le().catch($)}),document.querySelectorAll("#usage-tab-model [data-filter-apply]").forEach(M=>{M.onclick=()=>{c.usageFilter.modelQ=document.getElementById("uf-model")?.value?.trim()||"",c.usageFilter.modelPage=0,le().catch($)}}),document.querySelectorAll("#usage-tab-model [data-filter-reset]").forEach(M=>{M.onclick=()=>{c.usageFilter.modelQ="",c.usageFilter.modelPage=0,le().catch($)}}),document.querySelectorAll("#usage-tab-key [data-filter-apply]").forEach(M=>{M.onclick=()=>{c.usageFilter.keyQ=document.getElementById("uf-key")?.value?.trim()||"",c.usageFilter.keyActive=document.getElementById("uf-active")?.value||"",c.usageFilter.keyPage=0,le().catch($)}}),document.querySelectorAll("#usage-tab-key [data-filter-reset]").forEach(M=>{M.onclick=()=>{c.usageFilter.keyQ="",c.usageFilter.keyActive="",c.usageFilter.keyPage=0,le().catch($)}})}function ba(a){const s=a.versionStatus||(a.updateAvailable?"update_available":a.latest?"up_to_date":"unknown");return s==="update_available"?{badge:`<span class="badge warn" title="${t(e("system.statusHintUpdate"))}">${t(e("system.badgeUpdate"))}</span>`,hint:e("system.statusHintUpdate")}:s==="ahead"?{badge:`<span class="badge pending" title="${t(e("system.statusHintAhead"))}">${t(e("system.badgeAhead"))}</span>`,hint:e("system.statusHintAhead")}:s==="up_to_date"?{badge:`<span class="badge success" title="${t(e("system.statusHintOk"))}">${t(e("system.badgeOk"))}</span>`,hint:e("system.statusHintOk")}:{badge:`<span class="badge pending" title="${t(e("system.statusHintUnknown"))}">${t(e("system.badgeUnknown"))}</span>`,hint:e("system.statusHintUnknown")}}function Ms(a){return e(a==="git"?"system.channelGit":a==="npm-global"?"system.channelNpmGlobal":a==="npm-local"?"system.channelNpmLocal":"system.channelUnknown")}function qs(a){return a==="required"?e("system.levelRequired"):a==="recommended"?e("system.levelRecommended"):a==="optional"?e("system.levelOptional"):a==="bundled"?e("system.levelBundled"):a||"—"}function Ts(a){return a.installed?a.ok?`<span class="badge success">${t(e("system.softOk"))}</span>`:`<span class="badge warn">${t(e("system.softWarn"))}</span>`:a.level==="required"||a.level==="bundled"?`<span class="badge error">${t(e("system.softMissing"))}</span>`:`<span class="badge pending">${t(e("system.softMissing"))}</span>`}function ya(a){return a==="up"?`<span class="badge success">${t(e("system.up"))}</span>`:`<span class="badge error">${t(e("system.down"))}</span>`}async function wt(){const{data:a}=await S("/system");if(c.page!=="system")return;const s=a.version||{},n=ba(s),o=a.software||{checks:[],allRequiredOk:!0},i=o.checks||[],r=c.systemTab==="package"||c.systemTab==="env"?c.systemTab:"software";c.systemTab=r;const d=i.map(w=>`
      <tr>
...
`))}</textarea>
    ...
    </tr>`).join(""):`<tr class="empty-row"><td colspan="5"><div class="data-empty"><strong>${t(e("ddos.emptyEvents"))}</strong></div></td></tr>`}async function Z(a={}){const s=!!a.soft&&document.getElementById("ddos-root"),n=document.querySelector(".main"),o=n?n.scrollTop:0;_e&&(clearInterval(_e),_e=null);const i=[S("/ddos/connections"),S("/ddos/blacklist"),S("/ddos/stats"),S("/ddos/events")];s||i.push(S("/ddos/policy"));const r=await Promise.all(i),[d,u,m,p]=r,l=s?null:r[4],b=c.ddosFilter,h=b.pageSize;let f=d.data?.active||[],v=d.data?.recent||[],y=u.data||[];const w=m.data||{},L=p.data||[],B=l?.data||c._ddosPolicyCache||null,A=l?.presets||c._ddosPresetsCache||null;B&&(c._ddosPolicyCache=B),A&&(c._ddosPresetsCache=A);const M=(c._ddosPolicyCache?.whitelist||[]).map(String);if(b.liveQ.trim()){const I=b.liveQ.trim().toLowerCase(),Q=j=>[j.ip,j.path,j.method,j.apiKeyName,j.apiKeyPrefix].filter(Boolean).some(E=>String(E).toLowerCase().includes(I));f=f.filter(Q),v=v.filter(Q)}if(b.banQ.trim()){const I=b.banQ.trim().toLowerCase();y=y.filter(Q=>String(Q.ip||"").toLowerCase().includes(I)||String(Q.reason||"").toLowerCase().includes(I))}b.banSource&&(y=y.filter(I=>I.source===b.banSource));const H=f.slice(b.livePage*h,b.livePage*h+h),G=y.slice(b.banPage*h,b.banPage*h+h),V=H.map(I=>`
    <tr>
      <td class="cell-primary">${t(I.ip)}</td>
      <td>${t(I.method)}</td>
      <td class="muted" style="max-width:220px;word-break:break-all">${t(I.path)}</td>
      <td>${t(I.apiKeyName||I.apiKeyPrefix||"—")}</td>
      <td><span class="badge pending">${t(e("status.active"))}</span></td>
      <td>${It(Date.now()-I.startedAt)}</td>
      <td><div class="row-actions"><button class="btn danger sm" data-ban="${t(I.ip)}">${t(e("ddos.ban"))}</button></div></td>
    </tr>`).join(""),g=v.slice(0,40).map(I=>`
    <tr>
      <td class="cell-primary">${t(I.ip)}</td>
      <td>${t(I.method)} ${t(I.path)}</td>
      <td>${I.statusCode??"—"}</td>
      <td>${It(I.durationMs)}</td>
      <td><div class="row-actions"><button class="btn danger sm" data-ban="${t(I.ip)}">${t(e("ddos.ban"))}</button></div></td>
    </tr>`).join(""),k=G.map(I=>`
    <tr>
      <td class="cell-primary">${t(I.ip)}</td>
      <td>${t(I.reason||"—")}</td>
      <td><span class="badge pending">${t(Gt(I.source))}</span></td>
      <td>${I.expiresAt?ee(I.expiresAt):t(e("ddos.permanent"))}</td>
      <td><div class="row-actions"><button class="btn secondary sm" data-unban="${t(I.ip)}">${t(e("ddos.unban"))}</button></div></td>
    </tr>`).join(""),x=(w.topIps||[]).map(I=>`<tr><td class="cell-primary">${t(I.ip)}</td><td>${I.requests}</td>
      <td><div class="row-actions"><button class="btn danger sm" data-ban="${t(I.ip)}">${t(e("ddos.ban"))}</button></div></td></tr>`).join(""),_=Cs(L),R=`<tr class="empty-row"><td colspan="7"><div class="data-empty"><strong>${t(e("ddos.emptyLive"))}</strong></div></td></tr>`,P=`<tr class="empty-row"><td colspan="5"><div class="data-empty"><strong>${t(e("common.empty"))}</strong></div></td></tr>`,O=`<tr class="empty-row"><td colspan="5"><div class="data-empty"><strong>${t(e("ddos.emptyBan"))}</strong></div></td></tr>`,K=`<tr class="empty-row"><td colspan="3"><div class="data-empty"><strong>${t(e("common.empty"))}</strong></div></td></tr>`,U=["","manual","auto-auth","auto-rate","auto-conn","auto-velocity","auto-escalate"].map(I=>I?`<option value="${I}" ${b.banSource===I?"selected":""}>${t(Gt(I))}</option>`:`<option value="">${t(e("common.all"))}</option>`).join("");if(s){const I=(j,E)=>{const D=document.getElementById(j);D&&(D.innerHTML=E)},Q=(j,E)=>{const D=document.getElementById(j);D&&(D.textContent=E)};Q("ddos-stat-active",String(w.activeConnections??f.length)),Q("ddos-stat-rate",String(w.rateLimitedHits??0)),Q("ddos-stat-blocked",String(w.blockedHits??0)),Q("ddos-stat-ban",String(y.length)),Q("ddos-stat-auto",String(w.autoBanTotal??0)),Q("ddos-tab-count-live",String(f.length)),Q("ddos-tab-count-ban",String(y.length)),Q("ddos-tab-count-events",String(L.length)),I("ddos-live-body",V||R),I("ddos-recent-body",g||P),I("ddos-ban-body",k||O),I("ddos-top-body",x||K),I("ddos-events-body",_),w.policySummary&&ca(!!w.policySummary.autoBanEnabled),$a(),n&&(n.scrollTop=o)}else{const I=B||{autoBanEnabled:!0,rateLimitWindowMs:6e4,rateLimitMax:120,rateLimitIpMax:60,chatBurstWindowMs:1e4,chatBurstMax:20,autoAuthEnabled:!0,failedAuthThreshold:20,failedAuthWindowMs:3e5,authBanDurationMs:6e5,autoRateEnabled:!0,rateHitThreshold:30,rateHitWindowMs:6e4,rateBanDurationMs:9e5,autoConnEnabled:!0,maxConcurrentPerIp:20,connBanDurationMs:6e5,autoVelocityEnabled:!0,velocityMaxRequests:200,velocityWindowMs:6e4,velocityBanDurationMs:6e5,escalateEnabled:!0,escalateAfterBans:3,escalateDurationMs:864e5,whitelist:["127.0.0.1","::1"],proxyTrustHops:1,proxyIpSource:"auto",trustedProxies:["127.0.0.1","::1"]},Q=!!I.autoBanEnabled,j=b.tab==="live"||b.tab==="blacklist"||b.tab==="events"||b.tab==="policy"?b.tab:"policy";c.ddosFilter.tab=j;const E=`
    ...
    <div class="panel data-table-panel ddos-stack-panel">
      ...
      <table class="data-table">
        ...
        <tbody id="ddos-live-body">${V||R}</tbody>
      </table>
      </div>
    ...
    <div class="panel data-table-panel ddos-stack-panel">
      ...
      <table class="data-table">
        ...
        <tbody id="ddos-recent-body">${g||P}</tbody>
      </table>
      </div>
    </div>`,Me=`
    <div class="panel data-filter-panel ddos-filter-panel">
      ...
      <div class="data-filter">
        <div class="data-filter-grid">
          ...
          <label>${t(e("ddos.source"))}
            <select id="ddos-ban-source">${U}</select>
          </label>
        </div>
        <div class="data-filter-actions">
    ...
    <div class="panel data-table-panel ddos-stack-panel">
      <div class="panel-h"><strong>${t(e("ddos.blacklist"))}</strong>
        <span class="muted">${t(q("common.pagerTotal",{n:y.length}))}</span>
      </div>
      ...
      <table class="data-table">
        ...
        <tbody id="ddos-ban-body">${k||O}</tbody>
      </table>
      </div>
    ...
    <div class="panel data-table-panel ddos-stack-panel">
      ...
      <table class="data-table">
        ...
        <tbody id="ddos-top-body">${x||K}</tbody>
      </table>
      </div>
    </div>`;document.getElementById("app").innerHTML=ne(`
    <div id="ddos-root" class="${Q?"":"is-feature-off"}">
    <div class="topbar">
      <h2>${t(e("ddos.title"))}</h2>
      <div class="toolbar">
        ${la({id:"ddos-master-autoban",on:Q,onLabel:e("ddos.masterOn"),offLabel:e("ddos.masterOff"),title:e("ddos.autoBanMasterHint")})}
    ...
    <div class="feature-off-banner" id="ddos-disabled-banner" ${Q?"hidden":""} role="status">
      <strong>${t(e("common.featureOff"))}</strong>
    ...
    <div class="usage-tabs-panel panel ddos-tabs-panel">
      <div class="seg-tabs" role="tablist" aria-label="${t(e("ddos.title"))}">
        <button type="button" role="tab" class="seg-tab ${j==="policy"?"is-active":""}" data-ddos-tab="policy" aria-selected="${j==="policy"}">
          ${t(e("ddos.tabPolicy"))}
        </button>
        <button type="button" role="tab" class="seg-tab ${j==="live"?"is-active":""}" data-ddos-tab="live" aria-selected="${j==="live"}">
          ${t(e("ddos.tabLive"))}
          <span class="seg-tab-count" id="ddos-tab-count-live">${f.length}</span>
        </button>
        <button type="button" role="tab" class="seg-tab ${j==="blacklist"?"is-active":""}" data-ddos-tab="blacklist" aria-selected="${j==="blacklist"}">
          ${t(e("ddos.tabBlacklist"))}
          <span class="seg-tab-count" id="ddos-tab-count-ban">${y.length}</span>
        </button>
        <button type="button" role="tab" class="seg-tab ${j==="events"?"is-active":""}" data-ddos-tab="events" aria-selected="${j==="events"}">
          ${t(e("ddos.tabEvents"))}
          <span class="seg-tab-count" id="ddos-tab-count-events">${L.length}</span>
        </button>
      </div>
      <div class="usage-tab-body">
        <div class="usage-tab-pane ddos-tab-pane ddos-tab-pane-policy" id="ddos-tab-policy" ${j==="policy"?"":"hidden"}>
          ${D}
        </div>
        <div class="usage-tab-pane ddos-tab-pane ddos-tab-pane-stack" id="ddos-tab-live" ${j==="live"?"":"hidden"}>
          ${de}
        </div>
        <div class="usage-tab-pane ddos-tab-pane ddos-tab-pane-stack" id="ddos-tab-blacklist" ${j==="blacklist"?"":"hidden"}>
          ${Me}
        </div>
        <div class="usage-tab-pane ddos-tab-pane ddos-tab-pane-stack" id="ddos-tab-events" ${j==="events"?"":"hidden"}>
          ${Fe}
        </div>
      </div>
    </div>
    </div>
  `),oe(),$a(!0,M),document.querySelectorAll("[data-ddos-tab]").forEach(te=>{te.addEventListener("click",()=>{const fe=te.getAttribute("data-ddos-tab")||"policy",re=fe==="live"||fe==="blacklist"||fe==="events"||fe==="policy"?fe:"policy";c.ddosFilter.tab!==re&&(c.ddosFilter.tab=re,Z().catch($))})}),document.getElementById("ddos-live-filter-apply")?.addEventListener("click",()=>{c.ddosFilter.liveQ=document.getElementById("ddos-live-q")?.value?.trim()||"",c.ddosFilter.livePage=0,Z().catch($)}),document.getElementById("ddos-live-filter-reset")?.addEventListener("click",()=>{c.ddosFilter.liveQ="",c.ddosFilter.livePage=0,Z().catch($)}),document.getElementById("ddos-ban-filter-apply")?.addEventListener("click",()=>{c.ddosFilter.banQ=document.getElementById("ddos-ban-q")?.value?.trim()||"",c.ddosFilter.banSource=document.getElementById("ddos-ban-source")?.value||"",c.ddosFilter.banPage=0,Z().catch($)}),document.getElementById("ddos-ban-filter-reset")?.addEventListener("click",()=>{c.ddosFilter.banQ="",c.ddosFilter.banSource="",c.ddosFilter.banPage=0,Z().catch($)}),document.getElementById("ddoslive-prev")?.addEventListener("click",()=>{c.ddosFilter.livePage=Math.max(0,b.livePage-1),Z().catch($)}),document.getElementById("ddoslive-next")?.addEventListener("click",()=>{(b.livePage+1)*h<f.length&&(c.ddosFilter.livePage+=1,Z().catch($))}),document.getElementById("ddosban-prev")?.addEventListener("click",()=>{c.ddosFilter.banPage=Math.max(0,b.banPage-1),Z().catch($)}),document.getElementById("ddosban-next")?.addEventListener("click",()=>{(b.banPage+1)*h<y.length&&(c.ddosFilter.banPage+=1,Z().catch($))});const pe=document.querySelector(".main");pe&&(pe.onscroll=()=>{c._ddosScrollPauseUntil=Date.now()+4e3})}!Re&&c.page==="ddos"&&(_e=setInterval(()=>{c.page!=="ddos"||Re||c._ddosScrollPauseUntil&&Date.now()<c._ddosScrollPauseUntil||Z({soft:!0}).catch(()=>{})},2e3))}function $a(a=!1,s=[]){const n=s.length?s:c._ddosPolicyCache?.whitelist||[],o=async i=>{if(!i)return;const r=n.some(d=>String(d)===i||String(d).startsWith(i));await X({message:e(r?"ddos.banWhitelistWarn":"ddos.banConfirm"),variant:"danger",confirmText:e("ddos.ban")})&&(await S("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:i,reason:e("ddos.banReasonDefault"),ttlSeconds:null})}),Z({soft:!0}).catch($))};if(document.querySelectorAll("[data-ban]").forEach(i=>{i.onclick=()=>o(i.dataset.ban)}),document.querySelectorAll("[data-unban]").forEach(i=>{i.onclick=async()=>{await X({message:e("ddos.unbanConfirm"),variant:"danger",confirmText:e("ddos.unban")})&&(await S(`/ddos/blacklist/${encodeURIComponent(i.dataset.unban)}`,{method:"DELETE"}),Z({soft:!0}).catch($))}}),a){document.getElementById("ban-add").onclick=async()=>{const r=document.getElementById("ban-ip").value.trim();if(!r||n.some(m=>String(m)===r)&&!await X({message:e("ddos.banWhitelistWarn"),variant:"danger",confirmText:e("ddos.ban")}))return;const u=document.getElementById("ban-ttl").value;await S("/ddos/blacklist",{method:"POST",body:JSON.stringify({ip:r,reason:document.getElementById("ban-reason").value.trim()||void 0,ttlSeconds:u?Number(u):null})}),Z({soft:!0}).catch($)},document.getElementById("ddos-refresh").onclick=()=>Z({soft:!1}).catch($),document.getElementById("ddos-pause").onclick=()=>{Re=!Re;const r=document.getElementById("ddos-pause");r&&(r.textContent=e(Re?"ddos.resume":"ddos.pause")),Re||Z({soft:!0}).catch($)},document.getElementById("ddos-master-autoban")?.addEventListener("click",async()=>{const r=!Ue("ddos-master-autoban");We("ddos-master-autoban",r,e("ddos.masterOn"),e("ddos.masterOff"));const d=document.getElementById("dp-autoBanEnabled");d&&(d.type==="checkbox"?d.checked=r:d.value=r?"1":"0"),ca(r),Qe("ddos-root",!r),Ge("ddos-disabled-banner",!r),qe();try{const u=Qt(),m=await S("/ddos/policy",{method:"PUT",body:JSON.stringify(u)});c._ddosPolicyCache=m.data,qe()}catch(u){We("ddos-master-autoban",!r,e("ddos.masterOn"),e("ddos.masterOff")),Qe("ddos-root",r),Ge("ddos-disabled-banner",r),$(u)}});const i=document.getElementById("ddos-policy-panel");i?.addEventListener("input",()=>qe()),i?.addEventListener("change",()=>qe()),document.querySelectorAll("[data-ddos-preset]").forEach(r=>{r.onclick=()=>{const d=r.dataset.ddosPreset;if(d==="custom")return;const u=c._ddosPresetsCache?.[d];u&&Ft(u)}}),qe(),document.getElementById("dp-save")?.addEventListener("click",async()=>{try{const r=Qt(),d=await S("/ddos/policy",{method:"PUT",body:JSON.stringify(r)});c._ddosPolicyCache=d.data,Ft(d.data),qe(),await ce({title:e("ddos.policyTitle"),message:e("ddos.policySaved")}),Z({soft:!0}).catch($)}catch(r){$(r)}}),document.getElementById("dp-reset")?.addEventListener("click",async()=>{if(await X({message:e("ddos.confirmReset"),variant:"danger",confirmText:e("ddos.resetPolicy")}))try{const r=await S("/ddos/policy/reset",{method:"POST"});c._ddosPolicyCache=r.data,Ft(r.data),qe(),await ce({title:e("ddos.policyTitle"),message:e("ddos.policyReset")}),Z({soft:!0}).catch($)}catch(r){$(r)}})}}function gt(a){return e(a==="pm2"?"pm2.runnerPm2":a==="gctoac"?"pm2.runnerGctoac":a==="none"?"pm2.runnerNone":"pm2.runnerUnknown")}function Jt(a){if(!a)return"";const s=a.messageKey;if(s&&typeof s=="string"){if(s==="pm2.msgOk")return"";const o=a.messageParams||{},i=q(s,o);if(i&&i!==s)return i}const n=a.message||"";return!n||n==="ok"?"":n}function ka(a=10){const s=Math.max(1,Number(a)||10)*1e3;window.setTimeout(()=>{try{window.location.reload()}catch{window.location.href=window.location.href}},s)}function Ls(a,s){const n=s?.messageKey||(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac");let o=Jt({messageKey:n,messageParams:s?.messageParams,message:void 0});o||(o=e(a==="pm2"?"pm2.msgSwitchPm2":"pm2.msgSwitchGctoac"));const i=s?.port||s?.messageParams?.port||(typeof location<"u"&&location.port?location.port:"3847");return[o,q("pm2.portAfterRestart",{port:i}),q("pm2.autoRefreshIn",{n:10})].filter(Boolean).join(`
...
${u}`);const p=m?.files||[],l=p.length?p.filter(P=>P.exists).map(P=>`${P.label}: ${P.size<1024?P.size+" B":Math.round(P.size/1024)+" KB"}`).join(" · "):"",b=m?.maxBytes?Math.round(m.maxBytes/(1024*1024)):5,h=m?.keepBytes?Math.round(m.keepBytes/1024):512,f=n?.status||"—",v=f==="online"?e("pm2.statusOnline"):f==="errored"?e("pm2.statusErrored"):f==="stopped"?e("pm2.statusStopped"):f,y=f==="online"?`<span class="badge success">${t(v)}</span>`:f==="errored"?`<span class="badge error">${t(v)}</span>`:t(v),w=s.available,L=s.available&&n,B=s.runner||"unknown",A=d&&f!=="errored"&&s.available!==!1&&s.messageKey!=="pm2.msgErrored",M=Hs(o.env_extra),H=c.pm2Tab==="port"||c.pm2Tab==="config"||c.pm2Tab==="logs"||c.pm2Tab==="runner"?c.pm2Tab:"runner";c.pm2Tab=H;const G=`
    <div class="grid pm2-kpi-grid" id="pm2-kpi-grid">
      <div class="card">
        ...
        <div class="muted card-sub">${_t(B)}</div>
      </div>
      <div class="card">
    ...
    <div class="panel data-table-panel pm2-section-panel">
      ...
      <div class="panel-pad">
        <div class="grid">
          <div class="card"><div class="label">${t(e("pm2.currentRunner"))}</div><div class="value value-sm">${_t(B)}</div></div>
    ...
    <div class="panel data-table-panel pm2-section-panel pm2-logs-panel">
      ...
      <div class="panel-pad">
        <p class="muted" style="margin:0 0 8px;font-size:0.82rem">
          ${t(q("pm2.logsAutoTrim",{maxMb:b,keepKb:h}))}
          ${l?` · ${t(l)}`:""}
        </p>
    ...
    <div class="topbar">
      ...
      <div class="toolbar">
        ...
        <button class="btn secondary sm" id="pm2-reload" ${!L||n?.status!=="online"?"disabled":""}>${t(e("pm2.reload"))}</button>
      </div>
    </div>
    ${we([e("pm2.hint")])}
    ${d?`<div class="error-box${A?" warn-box":""}">${t(d)}</div>`:s.available?"":`<div class="error-box">${t(e("pm2.unavailable"))}</div>`}
    ${G}
  ...
  `),oe(),document.querySelectorAll("[data-pm2-tab]").forEach(P=>{P.addEventListener("click",()=>{const O=P.getAttribute("data-pm2-tab")||"runner",K=O==="port"||O==="config"||O==="logs"||O==="runner"?O:"runner";c.pm2Tab!==K&&(c.pm2Tab=K,Ee().catch($))})}),document.getElementById("pm2-logs-refresh")?.addEventListener("click",()=>{c.pm2Tab="logs",Ee().catch($)}),document.getElementById("pm2-logs-clear")?.addEventListener("click",async()=>{if(await X({message:e("pm2.confirmClearLogs"),variant:"danger",confirmText:e("pm2.clearLogs")}))try{const O=(await S("/pm2/logs/clear",{method:"POST",body:JSON.stringify({which:"all"})})).data?.cleared?.length||0;await ce({message:q("pm2.logsCleared",{n:O})}),Ee().catch($)}catch(P){$(P)}});const _=async P=>{if(await X({message:e(P==="pm2"?"pm2.confirmSwitchPm2":"pm2.confirmSwitchGctoac"),variant:"confirm",confirmText:e(P==="pm2"?"pm2.switchToPm2":"pm2.switchToGctoac")}))try{const K=await S("/pm2/switch",{method:"POST",body:JSON.stringify({mode:P})}),U=K?.data||K||{},I=Ls(P==="pm2"?"pm2":"gctoac",U);ka(10),await ce({title:e("common.notice"),message:I,confirmText:e("common.ok")});try{window.location.reload()}catch{window.location.href=window.location.href}}catch(K){$(K)}};document.getElementById("pm2-refresh").onclick=()=>Ee().catch($),document.getElementById("pm2-switch-pm2").onclick=()=>_("pm2"),document.getElementById("pm2-switch-gctoac").onclick=()=>_("gctoac"),document.getElementById("pm2-start").onclick=()=>_("pm2"),document.getElementById("pm2-stop").onclick=async()=>{if(await X({message:e("pm2.confirmStop"),variant:"danger",confirmText:e("pm2.stop")}))try{await S("/pm2/stop",{method:"POST",body:"{}"}),Ee().catch($)}catch(P){$(P)}},document.getElementById("pm2-restart").onclick=async()=>{if(await X({message:e("pm2.confirmRestart"),variant:"confirm",confirmText:e("pm2.restart")}))try{await _("pm2")}catch(P){$(P)}},document.getElementById("pm2-reload").onclick=async()=>{try{await S("/pm2/reload",{method:"POST",body:"{}"}),Ee().catch($)}catch(P){$(P)}};const R=async P=>{try{const O={...Os(),restart:P};if(O.port==null){await ce({message:e("pm2.portInvalid")});return}const K=await S("/pm2/config",{method:"PUT",body:JSON.stringify(O)});if(K.data?.scheduled){const U=K.data.portChange?`
${q("pm2.portChangedMsg",{from:K.data.portChange.previous,to:K.data.portChange.port})}`:"",I=Jt(K.data.scheduled)||e("pm2.switchScheduled");ka(10),await ce({title:e("common.notice"),message:I+U+`
${q("pm2.autoRefreshIn",{n:10})}`});try{window.location.reload()}catch{window.location.href=window.location.href}}else await ce(K.data?.portChange?q("pm2.portSavedNeedRestart",{port:K.data.port}):e("pm2.configSaved")),Ee().catch($)}catch(O){$(O)}};document.getElementById("pm2-cfg-save").onclick=()=>R(!0),document.getElementById("pm2-cfg-save-only").onclick=()=>R(!1),document.getElementById("pm2-port-default")?.addEventListener("click",()=>{const P=document.getElementById("pm2-cfg-port");P&&(P.value="3847")}),document.getElementById("pm2-port-save")?.addEventListener("click",async()=>{const P=Number(document.getElementById("pm2-cfg-port")?.value);if(!Number.isFinite(P)||P<1||P>65535){await ce({message:e("pm2.portInvalid")});return}if(await X({message:q("pm2.confirmPortChange",{port:P}),variant:"confirm",confirmText:e("pm2.savePort")}))try{const O=await S("/pm2/config",{method:"PUT",body:JSON.stringify({port:P,restart:!0})}),K=O.data?.scheduled?.message||(O.data?.portChange?q("pm2.portChangedMsg",{from:O.data.portChange.previous,to:O.data.portChange.port}):e("pm2.configSaved"));await ce(K+`
`+q("pm2.portAfterRestart",{port:P}))}catch(O){$(O)}}),document.getElementById("pm2-cfg-reset").onclick=async()=>{if(await X({message:e("pm2.confirmReset"),variant:"danger",confirmText:e("pm2.resetConfig")}))try{await S("/pm2/config/reset",{method:"POST",body:"{}"}),Ee().catch($)}catch(P){$(P)}}}let Y=[],me=null,ae=[],nt=!1,Ae=0;const Te=new Map,N={keyId:"",model:"",reasoning:!0,systemPrompt:"",systemOpen:!1},C={mode:"full",recentN:6,summary:"",summaryAt:null,summarySourceCount:0},Fs=3,Ga=40,_s=20,Rs=2200,T={conversationId:null,historyPage:0,historyLimit:20,historyQ:"",historyTotal:0,historyItems:[],historyLoading:!1,historyOpenMobile:!1,saving:!1,saveQueued:!1,renamingId:null};let Rt=null;const Mt=10,ua=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;function Ns(a){const s=String(a||"").split(/[/\\]/).pop()||"",n=s.lastIndexOf(".");return n<0?"":s.slice(n).toLowerCase()}function qt(a){return qa.has(Ns(a))}function Tt(){return e("chat.formatsHint")}function Qa(){C.mode="full",C.recentN=6,C.summary="",C.summaryAt=null,C.summarySourceCount=0,Ae=0,Te.clear()}function Ks(a){const s=(a?.title||"").trim();if(s)return s;const n=(a?.preview||"").trim();return n||e("chat.untitled")}function js(){return Y.filter(a=>!a.streaming).map(a=>{const s={role:a.role,content:a.content||""};return a.reasoning&&(s.reasoning=a.reasoning),a.docs&&a.docs.length&&(s.docs=a.docs),a.error&&(s.error=!0),s})}function Us(){return{contextMode:C.mode,contextRecentN:C.recentN,summaryText:C.summary||"",summaryAt:C.summaryAt,summarySourceCount:C.summarySourceCount||0}}function Ws(a){a&&(C.mode=a.contextMode==="summary"||a.contextMode==="recent"?a.contextMode:"full",C.recentN=Math.min(40,Math.max(2,Number(a.contextRecentN)||6)),C.summary=(a.summaryText||"").trim(),C.summaryAt=a.summaryAt||null,C.summarySourceCount=Number(a.summarySourceCount)||0,C.mode==="summary"&&!C.summary&&(C.mode="full"))}function za(a){return a.reduce((s,n)=>s+(n.content||"").length+(n.reasoning||"").length,0)}function Ja(){const a=Y.filter(s=>!s.streaming);return a.length<2?!1:a.length>=Fs?!0:za(a)>=800}function Gs(){const a=(N.systemPrompt||"").trim(),s=Y.filter(d=>!d.streaming),n=Math.min(40,Math.max(2,Number(C.recentN)||6));let o=s.map(d=>({role:d.role,content:d.content||""})),i=a;if(C.mode==="summary"&&C.summary){const d=(it()==="zh-Hant"?`【先前對話摘要 — 僅供延續語境，完整記錄仍在用戶介面】
`:`[Prior conversation summary — full history remains in the UI]
`)+C.summary;i=i?`${i}

${d}`:d;const u=s.slice(C.summarySourceCount||0);o=(u.length?u:s.slice(-n)).slice(-n).map(p=>({role:p.role,content:p.content||""}))}else C.mode==="recent"&&(o=s.slice(-n).map(d=>({role:d.role,content:d.content||""})));const r=o.map(d=>({role:d.role,content:d.content}));return i&&r.unshift({role:"system",content:i}),r}function Qs(){return C.mode==="summary"&&C.summary?q("chat.ctxModeSummaryLabel",{n:C.recentN}):C.mode==="recent"?q("chat.ctxModeRecentLabel",{n:C.recentN}):e("chat.ctxModeFullLabel")}function Ye(){const a=document.getElementById("chat-compress");if(!a)return;const s=Y.some(i=>i.streaming),n=!!me||nt||s,o=Ja();a.disabled=n||!o,a.textContent=e(nt?"chat.compressing":"chat.compress"),a.title=e(o?"chat.compress":"chat.compressNeedMore"),Le(),ze()}function ze(){const a=document.getElementById("chat-ctx-mode"),s=document.getElementById("chat-ctx-n");if(a){const n=C.mode==="summary"&&!C.summary?"full":C.mode;a.value=n;const o=a.querySelector('option[value="summary"]');o&&(o.disabled=!C.summary)}s&&(s.value=String(C.recentN),s.disabled=C.mode==="full")}function Le(){const a=document.getElementById("chat-compress-banner");if(!a)return;const s=!!C.summary,n=Y.filter(r=>!r.streaming).length>40||za(Y)>6e4;if(!s&&C.mode==="full"&&!n){a.hidden=!0,a.innerHTML="";return}a.hidden=!1;const o=s?C.summary.length>160?`${C.summary.slice(0,159)}…`:C.summary:"",i=n?`<p class="chat-compress-warn">${t(e("chat.ctxLongHint"))}</p>`:"";a.innerHTML=`
    ...
    </div>`,document.getElementById("chat-summary-view")?.addEventListener("click",()=>{Va()})}async function Va(){if(!C.summary){F(e("chat.compressNeedSummary"));return}const a=C.summaryAt?ee(C.summaryAt):"—",s=Aa(C.summary);ve&&Ne(!1);const n=document.createElement("div");return n.className="ui-dialog-back",n.id="ui-dialog-back",n.dataset.cancelable="1",n.innerHTML=`
...
`)}async function Vs(){if(nt||me||Y.some(i=>i.streaming)){F(e("chat.compressBusy"));return}const a=Y.filter(i=>!i.streaming);if(!Ja()){F(e("chat.compressNeedMore"));return}if(!await X({title:e("chat.compress"),message:e("chat.compressConfirm"),variant:"confirm",confirmText:e("chat.compress")}))return;const s=mt();if(!s){F(e("chat.needKey"));return}Be(),nt=!0,Ye();const n=document.getElementById("chat-send");n&&(n.disabled=!0);const o=document.getElementById("chat-stream-status");o&&(o.hidden=!1,o.textContent=e("chat.compressing"));try{let i=zs(a);C.summary&&(i=(it()==="zh-Hant"?`先前摘要：
${C.summary}
...
Full conversation:
`)+i);const r=document.getElementById("chat-model")?.value||N.model||"grok-4.5",d=ma(),u={model:r,stream:!1,include_reasoning:!1,messages:[{role:"system",content:Js()},{role:"user",content:(it()==="zh-Hant"?`請為以下對話產生摘要（僅供之後回合作為語境，不會刪除用戶介面中的記錄）：

`:`Summarize the following conversation (for later context only; UI history is kept):

`)+i}]},m=ut();m&&(u.apiKeyId=m);const p=await fetch("/admin/api/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${s}`,"Content-Type":"application/json"},body:JSON.stringify(u)});if(!p.ok){const h=await p.text();let f=h;try{f=JSON.parse(h).error?.message||h}catch{}throw new Error(f||e("chat.compressFail"))}const l=await p.json();let b=l?.choices?.[0]?.message?.content||l?.choices?.[0]?.delta?.content||"";if(typeof b!="string"&&(b=String(b||"")),b=b.trim().replace(/^【對話摘要】\s*/u,"").replace(/^\[Conversation summary\]\s*/i,""),!b)throw new Error(e("chat.compressFail"));C.summary=b,C.summaryAt=new Date().toISOString(),C.summarySourceCount=a.length,C.mode="summary",Le(),ze(),F(""),o&&(o.hidden=!1,o.textContent=e("chat.compressOk"),setTimeout(()=>{const h=document.getElementById("chat-stream-status");h&&h.textContent===e("chat.compressOk")&&(h.hidden=!0,h.textContent="")},2800)),await rt().catch(()=>{}),await Va()}catch(i){F(i.message||e("chat.compressFail"))}finally{nt=!1,Ye(),n&&(n.disabled=!1),o&&o.textContent===e("chat.compressing")&&(o.hidden=!0,o.textContent="")}}function Vt(a){T.historyOpenMobile=!!a,document.body.classList.toggle("chat-history-open",T.historyOpenMobile)}function Xt(){Vt(!1)}async function De(){if(c.key){T.historyLoading=!0,Ie();try{const a=T.historyPage*T.historyLimit,s=new URLSearchParams({limit:String(T.historyLimit),offset:String(a)});T.historyQ.trim()&&s.set("q",T.historyQ.trim());const n=await S(`/conversations?${s}`);T.historyItems=n.data||[],T.historyTotal=n.total??0}catch(a){T.historyItems=[],T.historyTotal=0,console.warn(a)}finally{T.historyLoading=!1,Ie()}}}function Ie(){const a=document.getElementById("chat-history-list"),s=document.getElementById("chat-history-pager");if(a){if(T.historyLoading&&!T.historyItems.length?a.innerHTML=`<li class="chat-history-empty">${t(e("common.loading"))}</li>`:T.historyItems.length?a.innerHTML=T.historyItems.map(n=>{const o=T.conversationId===n.id?" is-active":"",i=Ks(n),r=n.title&&n.preview&&n.preview!==n.title?n.preview:n.model||q("chat.msgs",{n:n.messageCount||0}),d=T.renamingId===n.id,u=i,m=d?`<input type="text" class="chat-history-title-input" data-title-input="${t(n.id)}" value="${t(u)}" maxlength="120" placeholder="${t(e("chat.renamePh"))}" aria-label="${t(e("chat.renamePh"))}" />
            <span class="preview">${t(r||"—")}</span>
            <span class="meta"><span>${t(ee(n.updatedAt))}</span></span>`:`<span class="title" data-title-label="${t(n.id)}" title="${t(e("chat.rename"))}">${t(i)}</span>
            <span class="preview">${t(r||"—")}</span>
            <span class="meta"><span>${t(ee(n.updatedAt))}</span></span>`,p=d?`<div class="chat-history-item${o} is-editing" data-conv-body="${t(n.id)}">${m}</div>`:`<div class="chat-history-item${o}" data-open-conv="${t(n.id)}" role="button" tabindex="0" title="${t(i)}">${m}</div>`;return`
        ...
        </li>`}).join(""):a.innerHTML=`<li class="chat-history-empty">${t(e("chat.historyEmpty"))}</li>`,s){const n=T.historyLimit,o=Math.max(1,Math.ceil(T.historyTotal/n)||1),i=Math.min(T.historyPage+1,o),r=q("chat.historyPage",{n:i,total:o}),d=T.historyPage>0,u=(T.historyPage+1)*n<T.historyTotal;s.innerHTML=`
    ...
    `;const m=document.getElementById("chat-hist-prev"),p=document.getElementById("chat-hist-next");m&&(m.onclick=()=>{T.historyPage>0&&(T.historyPage-=1,De())}),p&&(p.onclick=()=>{(T.historyPage+1)*n<T.historyTotal&&(T.historyPage+=1,De())})}if(a.querySelectorAll("[data-open-conv]").forEach(n=>{const o=n.getAttribute("data-open-conv");if(!o)return;let i=null;const r=()=>{i&&(clearTimeout(i),i=null)};n.addEventListener("click",d=>{T.renamingId||d.target instanceof Element&&d.target.closest(".chat-history-item-actions")||(r(),i=setTimeout(()=>{i=null,!T.renamingId&&wa(o)},280))}),n.addEventListener("dblclick",d=>{d.preventDefault(),d.stopPropagation(),r(),Nt(o)}),n.addEventListener("keydown",d=>{(d.key==="Enter"||d.key===" ")&&(d.preventDefault(),T.renamingId||wa(o))})}),a.querySelectorAll("[data-title-label]").forEach(n=>{n.addEventListener("dblclick",o=>{o.preventDefault(),o.stopPropagation();const i=n.getAttribute("data-title-label");i&&Nt(i)})}),a.querySelectorAll("[data-rename-conv]").forEach(n=>{n.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation();const i=n.getAttribute("data-rename-conv");i&&Nt(i)})}),a.querySelectorAll("[data-del-conv]").forEach(n=>{n.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation();const i=n.getAttribute("data-del-conv");i&&Ys(i)})}),T.renamingId){const n=String(T.renamingId).replace(/\\/g,"\\\\").replace(/"/g,'\\"'),o=a.querySelector(`[data-title-input="${n}"]`);o instanceof HTMLInputElement&&(Xs(o,T.renamingId),requestAnimationFrame(()=>{o.isConnected&&(o.focus(),o.select())}))}}}function Nt(a){a&&(T.renamingId&&T.renamingId!==a&&(T.renamingId=null),T.renamingId=a,Ie())}function Xs(a,s){let n=!1;const o=async i=>{if(n)return;n=!0;const r=a.value;if(T.renamingId=null,!i){Ie();return}const d=String(r??"").trim().slice(0,120),u=T.historyItems.find(p=>p.id===s),m=u?(u.title||"").trim():"";if(d===m){Ie();return}u&&(u.title=d),Ie();try{await S(`/conversations/${s}`,{method:"PATCH",body:JSON.stringify({title:d})}),await De()}catch(p){F(p.message||e("chat.saveFail")),await De()}};a.addEventListener("keydown",i=>{i.stopPropagation(),i.key==="Enter"?(i.preventDefault(),o(!0)):i.key==="Escape"&&(i.preventDefault(),o(!1))}),a.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation()}),a.addEventListener("mousedown",i=>i.stopPropagation()),a.addEventListener("dblclick",i=>{i.preventDefault(),i.stopPropagation()}),a.addEventListener("blur",()=>{setTimeout(()=>o(!0),0)})}async function wa(a){(!a||me)&&me&&me.abort();try{F("");const s=await S(`/conversations/${a}`),n=s.data||s;T.conversationId=n.id,Ae=0,Te.clear(),Y=(n.messages||[]).filter(u=>!u.compressed).map(u=>({role:u.role,content:u.content||"",reasoning:u.reasoning||void 0,docs:u.docs,error:u.error})),ae=[],N.systemPrompt=n.systemPrompt||"",Ws(n),n.model&&(N.model=n.model),n.apiKeyId&&(N.keyId=n.apiKeyId);const o=document.getElementById("chat-system");o&&(o.value=N.systemPrompt);const i=document.getElementById("chat-system-wrap");i&&(i.hidden=!N.systemPrompt.trim()&&!N.systemOpen);const r=document.getElementById("chat-model");r&&n.model&&(r.value=n.model);const d=document.getElementById("chat-key-select");d&&n.apiKeyId&&[...d.options].some(m=>m.value===n.apiKeyId)&&(d.value=n.apiKeyId,N.keyId=n.apiKeyId),xe(),Oe(),Ie(),Le(),ze(),Xt()}catch(s){F(s.message||e("chat.loadFail"))}}function ut(){const a=ma();return!a||String(a).startsWith("admin-session:")||!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(a)?null:a}async function rt(){const a=js();if(!a.length&&!C.summary)return;if(T.saving){T.saveQueued=!0;return}T.saving=!0,T.saveQueued=!1,Be();const s={messages:a,model:N.model||null,systemPrompt:N.systemPrompt||"",apiKeyId:ut(),...Us()};try{if(T.conversationId)await S(`/conversations/${T.conversationId}`,{method:"PATCH",body:JSON.stringify(s)});else{if(!a.length)return;const n=await S("/conversations",{method:"POST",body:JSON.stringify({...s,title:""})}),o=n.data||n;T.conversationId=o.id}await De()}catch(n){console.warn(n)}finally{T.saving=!1,T.saveQueued&&(T.saveQueued=!1,rt().catch(()=>{}))}}async function Ys(a){if(await X({title:e("chat.deleteConversation"),message:e("chat.deleteConfirm"),variant:"danger",confirmText:e("chat.deleteConversation")}))try{await S(`/conversations/${a}`,{method:"DELETE"}),T.conversationId===a&&(T.conversationId=null,Y=[],ae=[],Qa(),xe(),Oe(),Le(),ze()),T.historyItems.length<=1&&T.historyPage>0&&(T.historyPage-=1),await De()}catch(s){F(s.message||e("common.requestFailed"))}}function Zs(a=!0){me&&me.abort(),Y=[],ae=[],T.conversationId=null,Qa(),a||(N.systemPrompt="",N.systemOpen=!1),xe(),Oe(),Ie(),Le(),ze()}function mt(){return c.key}function ma(){const s=document.getElementById("chat-key-select")?.value||N.keyId||"";return s&&s!=="session"?s:c.me?.id||""}function Be(){const a=document.getElementById("chat-key-select"),s=document.getElementById("chat-model"),n=document.getElementById("chat-reasoning"),o=document.getElementById("chat-system");a&&(N.keyId=a.value==="session"?"":a.value),s&&(N.model=s.value),n&&(N.reasoning=n.checked),o&&(N.systemPrompt=o.value)}function Oe(){const a=document.getElementById("chat-pending");if(a){if(!ae.length){a.innerHTML="",a.hidden=!0;return}a.hidden=!1,a.innerHTML=ae.map((s,n)=>`
      ...
      </div>`).join(""),a.querySelectorAll("[data-rm-doc]").forEach(s=>{s.onclick=()=>{const n=Number(s.getAttribute("data-rm-doc"));ae.splice(n,1),Oe()}})}}function en(a,s){return`${a}:${(s||"").length}:${(s||"").slice(0,40)}`}function xe(){const a=document.getElementById("chat-messages");if(!a)return;const s=a.scrollHeight-a.scrollTop-a.clientHeight<120,n=Y.some(l=>l.streaming),o=document.getElementById("chat-stream-status");if(o&&(o.hidden=!n,o.textContent=n?e("chat.streaming"):""),!Y.length){a.innerHTML=`
      <div class="chat-empty">
        <strong>${t(e("chat.emptyTitle"))}</strong>
        <p>${t(e("chat.emptyHint"))}</p>
      </div>`,Ye();return}const i=Y.length,r=Math.max(0,i-Ga);Ae>r&&(Ae=r);const d=Ae,u=Y.slice(d),m=d,p=m>0?`<div class="chat-load-older">
          <button type="button" class="btn secondary sm" id="chat-load-older">${t(q("chat.loadOlder",{n:m}))}</button>
        </div>`:"";a.innerHTML=p+u.map((l,b)=>{const h=d+b,f=l.role==="user"?"user":"assistant",v=l.role==="user"?e("chat.you"):e("chat.assistant"),y=l.docs&&l.docs.length?`<div class="chat-attach-list">${l.docs.map(_=>`<span class="chat-attach-chip" title="${t(_.name)}"><span>📎 ${t(_.name)}</span></span>`).join("")}</div>`:"",L=!!l.reasoning?`<details class="chat-reasoning" ${l.streaming||!l.content?"open":""}>
            <summary>${t(e("chat.reasoning"))}${l.streaming&&!l.content?` · ${t(e("chat.streaming"))}`:""}</summary>
            <pre>${t(l.reasoning)}</pre>
          </details>`:"";let B=l.content||"";!B&&l.streaming&&(B=l.reasoning?"":"…");const A=l.error?" error":"",M=l.streaming?" is-streaming":"",H=f==="assistant"&&!l.streaming&&!!B;let G;if(H){const _=en(h,B);if(Te.has(_))G=Te.get(_);else if(G=Aa(B),Te.set(_,G),Te.size>200){const R=Te.keys().next().value;Te.delete(R)}}else G=t(B);const V=!l.streaming&&B.length>Rs,g=`${H?"chat-content md":"chat-content"}${V?" is-collapsible":""}`,k=V?`<button type="button" class="btn ghost sm chat-expand-btn" data-expand="${h}">${t(e("chat.showMore"))}</button>`:"",x=B?`<button type="button" class="chat-copy-btn" data-copy-msg="${h}" title="${t(e("chat.copy"))}">${t(e("chat.copy"))}</button>`:"";return`<div class="chat-bubble ${f}${A}${M}" data-msg-idx="${h}">
...
✗ ${r}`,!0}const n=s.choices?.[0]?.delta||{};let o=!1;n.reasoning_content&&(a.reasoning=(a.reasoning||"")+n.reasoning_content,o=!0),(n.thought&&!n.reasoning_content||n.thought&&n.reasoning_content&&n.thought!==n.reasoning_content)&&(a.reasoning=(a.reasoning||"")+n.thought,o=!0),typeof n.content=="string"&&n.content.length&&(a.content=(a.content||"")+n.content,o=!0);const i=s.choices?.[0]?.message;return i&&(i.content&&!a.content&&(a.content=i.content,o=!0),i.reasoning_content&&!a.reasoning&&(a.reasoning=i.reasoning_content,o=!0)),o}function tn(a,s){const n=mt();return n?new Promise((o,i)=>{const r=new FormData;r.append("file",a,a.name);const d=ut();d&&r.append("apiKeyId",d);const u=new XMLHttpRequest;u.open("POST","/admin/api/documents"),u.setRequestHeader("Authorization",`Bearer ${n}`),u.upload.onprogress=m=>{if(s)if(m.lengthComputable&&m.total>0){const p=Math.min(100,Math.round(m.loaded/m.total*100));s({loaded:m.loaded,total:m.total,percent:p})}else s({loaded:m.loaded||0,total:0,percent:-1})},u.onload=()=>{let m=null;try{m=u.responseText?JSON.parse(u.responseText):null}catch{m=null}if(u.status<200||u.status>=300){const b=m?.error?.message||m?.message||u.responseText||u.statusText;i(new Error(b||e("chat.uploadFail")));return}const p=m?.data||m,l=p?.id;if(!l||typeof l!="string"){i(new Error(e("chat.uploadFail")));return}o({id:l,name:p.originalName||p.filename||a.name,mime:p.mimeType||a.type||"",size:p.sizeBytes??p.size??a.size??0})},u.onerror=()=>i(new Error(e("chat.uploadFail"))),u.onabort=()=>i(new Error(e("chat.uploadFail"))),u.send(r)}):Promise.reject(new Error(e("chat.needKey")))}function bt(a){const s=document.getElementById("chat-upload-progress");if(!s)return;const{visible:n,fileName:o,fileIndex:i,fileTotal:r,percent:d,indeterminate:u}=a;if(!n){s.hidden=!0,s.setAttribute("aria-hidden","true");return}s.hidden=!1,s.setAttribute("aria-hidden","false");const m=document.getElementById("chat-upload-label"),p=document.getElementById("chat-upload-bar"),l=document.getElementById("chat-upload-pct"),b=o||"",h=i||1,f=r||1;m&&(m.textContent=f>1?q("chat.uploadProgressMulti",{name:b,i:h,n:f}):q("chat.uploadProgress",{name:b}));const v=!!u||d<0;p&&(p.classList.toggle("is-indeterminate",v),v?p.style.width="40%":p.style.width=`${Math.max(0,Math.min(100,d))}%`),l&&(l.textContent=v?e("chat.uploading"):q("common.percent",{n:Math.max(0,Math.min(100,d))}))}function an(a){const s=Array.isArray(a)?a:[];if(!s.length)return{added:0,skipped:0};let n=0,o=0;const i=new Set(ae.map(r=>r.id));for(const r of s){if(ae.length>=Mt){o+=s.length-n-o;break}const d=r?.id,u=r?.name||r?.originalName||"";if(!d||!ua.test(String(d))){o+=1;continue}if(!qt(u)){o+=1;continue}if(i.has(d)){o+=1;continue}ae.push({id:d,name:u||d,mime:r.mime||r.mimeType||"",size:r.size??r.sizeBytes??0}),i.add(d),n+=1}return Oe(),{added:n,skipped:o}}async function sn(){if(!mt()){F(e("chat.needKey"));return}const a=ut(),s=Math.max(0,Mt-ae.length);if(s<=0){F(e("chat.tooManyFiles"));return}const n=new Map;let o=0;et({title:e("chat.libraryTitle"),subtitle:t(e("chat.librarySubtitle")),size:"md",bodyHtml:`
      ...
      <button type="button" class="btn sm" id="chat-lib-add" disabled>${t(e("chat.libraryAdd"))}</button>`});const i=document.getElementById("chat-lib-list"),r=document.getElementById("chat-lib-q"),d=document.getElementById("chat-lib-count"),u=document.getElementById("chat-lib-add");document.getElementById("chat-lib-cancel")?.addEventListener("click",()=>ue());const m=()=>{d&&(d.textContent=q("chat.librarySelected",{n:n.size})),u&&(u.disabled=n.size===0,u.textContent=n.size>0?`${e("chat.libraryAdd")} (${n.size})`:e("chat.libraryAdd"))},p=h=>{if(!i)return;const f=new Set(ae.map(y=>y.id)),v=(h||[]).filter(y=>qt(y.originalName));if(!v.length){i.innerHTML=`<div class="data-empty chat-lib-empty"><strong>${t(e("chat.libraryEmpty"))}</strong></div>`;return}i.innerHTML=v.map(y=>{const w=f.has(y.id),L=n.has(y.id),B=w&&!L;return`
          <label class="chat-lib-row ${w?"is-already":""} ${L?"is-selected":""}" data-id="${t(y.id)}">
            <input type="checkbox" data-lib-id="${t(y.id)}" ${L?"checked":""} ${B?"disabled":""} />
            <span class="chat-lib-meta">
          ...
          </label>`}).join(""),i.querySelectorAll("input[data-lib-id]").forEach(y=>{y.addEventListener("change",()=>{const w=y.getAttribute("data-lib-id"),L=v.find(A=>A.id===w);if(!L)return;if(y.checked){if(n.size>=s&&!n.has(w)){y.checked=!1,F(e("chat.tooManyFiles"));return}n.set(w,L)}else n.delete(w);const B=y.closest(".chat-lib-row");B&&B.classList.toggle("is-selected",y.checked),m()})})},l=async()=>{const h=++o;i&&(i.innerHTML=`<div class="muted chat-lib-status">${t(e("common.loading")||"…")}</div>`);try{const f=new URLSearchParams({limit:"50",offset:"0"});a&&f.set("apiKeyId",a);const v=(r?.value||"").trim();v&&f.set("q",v);const y=await S(`/documents?${f}`);if(h!==o)return;p(y.data||[])}catch(f){if(h!==o)return;i&&(i.innerHTML=`<div class="error-box">${t(f.message||e("chat.libraryLoadFail"))}</div>`)}};let b=null;r?.addEventListener("input",()=>{b&&clearTimeout(b),b=setTimeout(()=>l(),280)}),u?.addEventListener("click",()=>{const h=[...n.values()],{added:f}=an(h.map(v=>({id:v.id,name:v.originalName,mime:v.mimeType,size:v.sizeBytes})));ue(),f>0&&F("")}),m(),await l(),r?.focus()}async function Xa(a){const s=[...a||[]];if(!s.length)return;if(!mt()){F(e("chat.needKey"));return}const n=s.filter(u=>!qt(u.name)),o=s.filter(u=>qt(u.name));if(n.length&&(F(q("chat.formatsReject",{name:n.map(u=>u.name).join(", "),formats:Tt()})),!o.length))return;if(ae.length+o.length>Mt){F(e("chat.tooManyFiles"));return}const i=document.getElementById("chat-attach"),r=document.getElementById("chat-send");i&&(i.disabled=!0,i.textContent=e("chat.uploading")),r&&(r.disabled=!0);const d=o.length;try{let u=0;for(const m of o){if(ae.length>=Mt)break;u+=1,bt({visible:!0,fileName:m.name,fileIndex:u,fileTotal:d,percent:0,indeterminate:!1});const p=await tn(m,({percent:l})=>{bt({visible:!0,fileName:m.name,fileIndex:u,fileTotal:d,percent:l<0?0:l,indeterminate:l<0})});bt({visible:!0,fileName:m.name,fileIndex:u,fileTotal:d,percent:100,indeterminate:!1}),ae.some(l=>l.id===p.id)||ae.push(p),Oe()}n.length||F("")}catch(u){F(u.message||e("chat.uploadFail"))}finally{bt({visible:!1}),i&&(i.disabled=!1,i.textContent=e("chat.attach")),r&&(r.disabled=!1)}}function nn(){const a=c.me?.id||"",s=c.me?`${e("chat.useSessionKey")} · ${c.me.name||""} (${c.me.keyPrefix||""}…)`:e("chat.useSessionKey"),n=N.keyId||"session",o=(c.keys||[]).filter(r=>r.isActive!==!1),i=[`<option value="session" ${n==="session"||n===a||!n?"selected":""}>${t(s)}</option>`];for(const r of o){if(a&&r.id===a)continue;const d=`${r.name||"key"} · ${r.keyPrefix||""}… · ${r.role||""}/${r.mode||""}`;i.push(`<option value="${t(r.id)}" ${n===r.id?"selected":""}>${t(d)}</option>`)}return i.join("")}async function on(){await Promise.all([dt(!1),ct()]);const a=c.models||[];!N.model&&a.length&&(N.model=a[0]);const s=a.map(m=>`<option value="${t(m)}" ${N.model===m?"selected":""}>${t(m)}</option>`).join("");Vt(!1),document.getElementById("app").innerHTML=ne(`
      ...
      <div class="chat-body">
        ...
        <div class="chat-shell">
          <div class="chat-toolbar">
            ...
            <label class="check-inline" for="chat-reasoning">
              <input type="checkbox" id="chat-reasoning" ${N.reasoning!==!1?"checked":""} />
            ...
            <button type="button" class="btn ghost sm" id="chat-system-toggle" title="${t(e("chat.systemHint"))}">
              ${t(e("chat.systemPrompt"))}${N.systemPrompt?" ·":""}
            </button>
          </div>
          <div class="chat-system-wrap" id="chat-system-wrap" ${N.systemOpen||N.systemPrompt?"":"hidden"}>
            <label class="chat-system-label" for="chat-system">${t(e("chat.systemPrompt"))}
              <span class="hint">${t(e("chat.systemHint"))}</span>
            </label>
            <textarea id="chat-system" rows="3" placeholder="${t(e("chat.systemPlaceholder"))}">${t(N.systemPrompt||"")}</textarea>
          </div>
        ...
        <aside class="chat-history-rail" id="chat-history-rail" aria-label="${t(e("chat.history"))}">
          <div class="chat-history-head">
            ...
            <input type="search" id="chat-history-search" class="chat-history-search" placeholder="${t(e("chat.historySearch"))}" value="${t(T.historyQ)}" />
          </div>
...
✗ ${w.message||w}`,F(w.message||String(w)))}finally{p.streaming=!1,me=null,xe(),Ye(),h&&(h.disabled=!1),v&&(v.disabled=!1),y&&(y.disabled=!1),f&&(f.disabled=!0),rt().catch(()=>{})}}async function Lt(){const a=document.getElementById("app");try{if(!c.key){await Da();return}c.me||await Ha(),c.page==="dashboard"?await Ut():c.page==="chat"?await on():c.page==="chats"?await vt():c.page==="keys"?await Ke():c.page==="documents"?await st():c.page==="media"?await Pe():c.page==="audit"?await $t():c.page==="settings"?await ja():c.page==="apiFeatures"?await kt():c.page==="usage"?await le():c.page==="ddos"?await Z():c.page==="queue"?await se():c.page==="pm2"?await Ee():c.page==="system"?await wt():await Ut()}catch(s){a.innerHTML=ne(`<div class="error-box">${t(s.message)}</div>`),oe()}}let je=null;const z={tab:"overview",status:"",limit:20,offset:0};function At(a){return!a||a<0?"—":a<1e3?`${a}ms`:a<6e4?`${Math.round(a/1e3)}s`:a<36e5?`${Math.round(a/6e4)}m`:`${(a/36e5).toFixed(1)}h`}const ln=["enabled","globalConcurrency","perKeyConcurrency","maxQueueDepth","maxQueueDepthPerKey","fairness","defaultPriority","playgroundPriority","leaseMs","maxWaitMs"];function Ya(){return{relaxed:{enabled:!0,globalConcurrency:6,perKeyConcurrency:2,maxQueueDepth:200,maxQueueDepthPerKey:40,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:40,leaseMs:6e4,maxWaitMs:9e5},balanced:{enabled:!0,globalConcurrency:4,perKeyConcurrency:1,maxQueueDepth:100,maxQueueDepthPerKey:20,fairness:"weighted_round_robin",defaultPriority:100,playgroundPriority:50,leaseMs:45e3,maxWaitMs:6e5},strict:{enabled:!0,globalConcurrency:2,perKeyConcurrency:1,maxQueueDepth:40,maxQueueDepthPerKey:8,fairness:"fifo_global",defaultPriority:100,playgroundPriority:80,leaseMs:3e4,maxWaitMs:3e5}}}function Pa(a){if(!a)return{};const s={};for(const n of ln){const o=a[n];typeof o=="boolean"?s[n]=o:typeof o=="number"&&Number.isFinite(o)?s[n]=Math.round(o):typeof o=="string"?s[n]=o:o==null?s[n]=null:s[n]=o}return s}function Za(a,s){return JSON.stringify(Pa(a))===JSON.stringify(Pa(s))}function Yt(a){if(!a)return"custom";const s=Ya();for(const n of["relaxed","balanced","strict"])if(Za(a,s[n]))return n;return"custom"}function cn(a){return e(a==="relaxed"?"queue.presetRelaxed":a==="balanced"?"queue.presetBalanced":a==="strict"?"queue.presetStrict":"queue.presetCustom")}function es(a,{unsaved:s=!1}={}){const n=cn(a),o=a==="relaxed"?"relaxed":a==="balanced"?"balanced":a==="strict"?"strict":"custom",i=s?q("queue.presetFormLabel",{name:n}):q("queue.presetActiveLabel",{name:n});return`<span class="ddos-preset-badge is-${o}" id="queue-preset-badge" title="${t(i)}">${t(i)}</span>`}function ts(){return{enabled:document.getElementById("q-master-enabled")?Ue("q-master-enabled"):!0,globalConcurrency:Math.max(1,Math.min(64,Math.floor(W("qp-gconc",4)))),perKeyConcurrency:Math.max(1,Math.min(16,Math.floor(W("qp-kconc",1)))),maxQueueDepth:Math.max(1,Math.floor(W("qp-depth",100))),maxQueueDepthPerKey:Math.max(1,Math.floor(W("qp-depthk",20))),fairness:document.getElementById("qp-fair")?.value==="fifo_global"?"fifo_global":"weighted_round_robin",defaultPriority:Math.max(0,Math.min(1e3,Math.floor(W("qp-pri",100)))),playgroundPriority:Math.max(0,Math.min(1e3,Math.floor(W("qp-ppri",50)))),leaseMs:Math.max(5e3,Math.floor(W("qp-lease",45e3))),maxWaitMs:Math.max(5e3,Math.floor(W("qp-wait",6e5)))}}function Bt(a){We("q-master-enabled",a,e("queue.masterOn"),e("queue.masterOff")),Qe("queue-root",!a),Ge("queue-disabled-banner",!a);const s=document.getElementById("qk-pill-enabled");s&&(s.innerHTML=Ce(a,e("dash.on"),e("dash.off")))}function un(a){if(!a)return;const s=(o,i)=>{const r=document.getElementById(o);r&&(r.value=String(i))};Bt(a.enabled!==!1),s("qp-gconc",a.globalConcurrency),s("qp-kconc",a.perKeyConcurrency),s("qp-depth",a.maxQueueDepth),s("qp-depthk",a.maxQueueDepthPerKey);const n=document.getElementById("qp-fair");n&&(n.value=a.fairness||"weighted_round_robin"),s("qp-pri",a.defaultPriority),s("qp-ppri",a.playgroundPriority),s("qp-lease",a.leaseMs),s("qp-wait",a.maxWaitMs),ot()}function ot(){if(!document.getElementById("queue-policy-panel"))return;let a;try{a=ts()}catch{return}const s=Yt(a),n=Yt(c._queuePolicyCache||a),o=!Za(a,c._queuePolicyCache||a);document.querySelectorAll("[data-queue-preset]").forEach(d=>{const u=d.dataset.queuePreset;if(u==="custom"){const b=s==="custom";d.classList.toggle("is-active",b),d.setAttribute("aria-pressed",b?"true":"false"),d.disabled=!b;return}const m=u===s,p=u===n;d.classList.toggle("is-active",m),d.classList.toggle("is-saved",p&&!m),d.setAttribute("aria-pressed",m?"true":"false");const l=e(u==="relaxed"?"queue.presetRelaxed":u==="balanced"?"queue.presetBalanced":"queue.presetStrict");m&&p?d.innerHTML=`${t(l)} <span class="preset-tag">${t(e("queue.presetTagActive"))}</span>`:m&&o?d.innerHTML=`${t(l)} <span class="preset-tag preset-tag--draft">${t(e("queue.presetTagDraft"))}</span>`:p?d.innerHTML=`${t(l)} <span class="preset-tag preset-tag--saved">${t(e("queue.presetTagSaved"))}</span>`:d.textContent=l});const i=document.getElementById("queue-preset-badge");i&&(i.outerHTML=es(s,{unsaved:o&&s!==n}));const r=document.getElementById("queue-preset-hint");if(r){const d={relaxed:e("queue.presetRelaxedHint"),balanced:e("queue.presetBalancedHint"),strict:e("queue.presetStrictHint"),custom:e("queue.presetCustomHint")};r.textContent=d[s]||d.custom}}function mn(){document.querySelectorAll("[data-queue-preset]").forEach(a=>{a.dataset.queuePreset!=="custom"&&(a.onclick=()=>{const s=a.dataset.queuePreset,n=Ya()[s];n&&un(n)})}),["qp-gconc","qp-kconc","qp-depth","qp-depthk","qp-fair","qp-pri","qp-ppri","qp-lease","qp-wait"].forEach(a=>{const s=document.getElementById(a);s&&(s.addEventListener("change",()=>ot()),s.addEventListener("input",()=>ot()))}),ot()}function Ea(){return document.querySelector(".main")}function as(a){return a.map(s=>{const n=s.status==="queued"||s.status==="leased"||s.status==="running",o=s.status==="failed"||s.status==="dead"||s.status==="cancelled",i=s.startedAt||s.finishedAt?null:s.queuedAt?Date.now()-new Date(s.queuedAt).getTime():null;return`
    <tr data-q-row="${t(s.id)}">
      ...
      <td>
        ...
        ${i!=null&&s.status==="queued"?`<div class="cell-sub" data-q-wait>${t(e("queue.wait"))}: ${At(i)}</div>`:s.startedAt?`<div class="cell-sub">${t(e("queue.started"))}: ${ee(s.startedAt)}</div>`:""}
      </td>
      <td>
        ...
        </div>`,document.getElementById("q-filter-dead")?.addEventListener("click",()=>{z.status="dead",z.offset=0,z.tab="jobs",se().catch($)}),document.getElementById("q-purge-dlq")?.addEventListener("click",()=>{document.getElementById("q-purge")?.click()})):w.innerHTML="");const L=(g,k)=>{const x=document.getElementById(g);x&&(x.textContent=String(k??0))};L("q-tab-count-jobs",o),L("q-tab-count-dead",r);const B=document.getElementById("qk-jobs-meta");B&&(B.textContent=q("queue.jobsMeta",{n:o}));const A=document.querySelector("#queue-jobs-table tbody");if(A){const g=n.map(x=>`${x.id}|${x.status}|${x.priority}|${x.attempt}|${x.cancelRequested?1:0}|${x.errorMessage||""}|${x.startedAt||""}|${x.finishedAt||""}`).join(";"),k=as(n)||`<tr class="empty-row"><td colspan="9">
        <div class="data-empty">
          <div class="data-empty-icon">∅</div>
          <strong>${t(e("queue.empty"))}</strong>
        </div>
      </td></tr>`;if(A.dataset.qsig!==g){const x=document.querySelector("#queue-jobs-table .table-wrap"),_=x?.scrollLeft||0;A.dataset.qsig=g,A.innerHTML=k,ss(),x&&(x.scrollLeft=_)}else n.forEach(x=>{if(x.status!=="queued"||!x.queuedAt)return;const _=Date.now()-new Date(x.queuedAt).getTime(),R=String(x.id||"");let P=null;A.querySelectorAll("[data-q-row]").forEach(K=>{K.getAttribute("data-q-row")===R&&(P=K)});const O=P?.querySelector("[data-q-wait]");O&&(O.textContent=`${e("queue.wait")}: ${At(_)}`)})}if(document.querySelector("#queue-pager .data-pager-meta span")){const g=Math.max(1,Math.ceil((o||0)/z.limit)||1),k=Math.floor(z.offset/z.limit)+1,x=document.querySelectorAll("#queue-pager .data-pager-meta > span");x[0]&&(x[0].textContent=q("common.pagerTotal",{n:o||0})),x[1]&&(x[1].textContent=q("common.pagerPage",{n:k,total:g}));const _=document.getElementById("queue-prev"),R=document.getElementById("queue-next");_&&(_.disabled=z.offset<=0),R&&(R.disabled=z.offset+z.limit>=o)}const H=document.getElementById("q-pause");H&&(H.textContent=s.paused?e("queue.resume"):e("queue.pause"));const G=document.getElementById("q-drain");G&&(G.textContent=s.drainMode?e("queue.undrain"):e("queue.drainBtn"));const V=document.getElementById("q-master-enabled");V&&document.activeElement!==V&&Bt(s.enabled!==!1)}function xa(){je||(je=setInterval(()=>{if(c.page!=="queue"){clearInterval(je),je=null;return}const a=document.activeElement;a&&a.closest&&a.closest("#queue-policy-panel")&&(a.tagName==="INPUT"||a.tagName==="SELECT"||a.tagName==="TEXTAREA")||se({soft:!0}).catch(()=>{})},4e3))}async function se(a={}){const s=!!a.soft&&document.getElementById("queue-root");!s&&je&&(clearInterval(je),je=null);const n=Ea(),o=!s&&n?n.scrollTop:0,i=z,r=new URLSearchParams;r.set("limit",String(i.limit)),r.set("offset",String(i.offset)),i.status&&r.set("status",i.status);const[d,u,m]=await Promise.all([S("/queue/stats"),S(`/queue/jobs?${r}`),S("/queue/policy")]);if(c.page!=="queue")return;const p=d.data||{},l=m.data||p.policy||{},b=u.data||[],h=u.total??b.length,f=p.byStatus||{},v=p.dead??f.dead??0,y=p.leased??f.leased??0,w=p.running??f.running??0,L=p.queued??f.queued??0,B=p.depth??L+y+w,A=ns(l);if(c._queuePolicyCache={...l},s){pn({s:p,pol:l,jobs:b,total:h,by:f}),xa();return}z.tab||(z.tab="overview");const M=z.tab==="jobs"||z.tab==="policy"?z.tab:"overview";z.tab=M;const H=as(b),G=He({title:e("queue.filterTitle"),hint:e("queue.filterHint"),meta:q("queue.jobsMeta",{n:h}),gridHtml:`
      ...
      <th>${t(e("common.actions"))}</th>`,bodyHtml:H,colSpan:9,emptyText:e("queue.empty"),pagerHtml:ke({total:h,limit:i.limit,offset:i.offset,idPrefix:"queue"})}),g=l.fairness==="fifo_global"?e("queue.fifo"):e("queue.wrr"),k=l.enabled!==!1,x=(U,I,Q,j,E)=>`
    <div class="card">
      <div class="label">${t(U)}</div>
      <div class="value value-sm" id="${t(j)}">${I}</div>
      ${Q!=null&&Q!==""?`<div class="muted card-sub"${E?` id="${t(E)}"`:""}>${t(String(Q))}</div>`:""}
    </div>`,_=`
    <div class="grid queue-kpi-grid" id="queue-kpi-grid">
      ${x(e("queue.depth"),t(String(B)),q("queue.kpiDepthSub",{q:L,l:y}),"qk-depth","qk-depth-sub")}
      ${x(e("queue.activeJobs"),`${w}<span class="dash-kpi-den">/${l.globalConcurrency??"—"}</span>`,q("queue.kpiActiveSub",{n:p.workerActive??0}),"qk-running","qk-running-sub")}
      ${x(e("queue.queued"),t(String(L)),e("queue.kpiQueuedSub"),"qk-queued","qk-queued-sub")}
      ...
      ${x(e("queue.mode"),t(A),g,"qk-mode","qk-mode-sub")}
    </div>`,R=`
    ...
    ${G}
    <div id="queue-jobs-table" class="queue-jobs-table-host">${V}</div>`,O=`
  ...
  `),oe(),document.querySelectorAll("[data-queue-tab]").forEach(U=>{U.addEventListener("click",()=>{const I=U.getAttribute("data-queue-tab")||"overview";I!=="overview"&&I!=="jobs"&&I!=="policy"||z.tab!==I&&(z.tab=I,se().catch($))})}),o>0){const U=Ea();U&&(U.scrollTop=o,requestAnimationFrame(()=>{U.scrollTop=o}))}document.getElementById("q-master-enabled").onclick=async()=>{const U=!Ue("q-master-enabled");Bt(U);try{const I=await S("/queue/policy",{method:"PUT",body:JSON.stringify({enabled:U})});c._queuePolicyCache={...c._queuePolicyCache||{},...I.data||{enabled:U}},ot()}catch(I){Bt(!U),$(I)}},document.getElementById("q-pause").onclick=async()=>{await S(l.paused?"/queue/resume":"/queue/pause",{method:"POST",body:"{}"}),se().catch($)},document.getElementById("q-drain").onclick=async()=>{await S(l.drainMode?"/queue/undrain":"/queue/drain",{method:"POST",body:"{}"}),se().catch($)};const K=async()=>{await X({title:e("queue.purgeDead"),message:e("queue.purgeConfirm"),variant:"danger",confirmText:e("queue.purgeDead")})&&(await S("/queue/purge-dead",{method:"POST",body:"{}"}),se().catch($))};document.getElementById("q-purge").onclick=()=>K().catch($),document.getElementById("q-purge-dlq")?.addEventListener("click",()=>K().catch($)),document.getElementById("q-filter-dead")?.addEventListener("click",()=>{z.status="dead",z.offset=0,z.tab="jobs",se().catch($)}),document.querySelectorAll("[data-filter-apply]").forEach(U=>{U.onclick=()=>{z.status=document.getElementById("qf-status")?.value||"",z.offset=0,se().catch($)}}),document.querySelectorAll("[data-filter-reset]").forEach(U=>{U.onclick=()=>{z.status="",z.offset=0,se().catch($)}}),Ze("queue",z,()=>se().catch($)),document.getElementById("qp-save").onclick=async()=>{const U=ts();await S("/queue/policy",{method:"PUT",body:JSON.stringify(U)}),c._queuePolicyCache={...c._queuePolicyCache||{},...U},F(""),se().catch($)},mn(),ss(),xa()}c.page=ls();(!location.hash||location.hash==="#"||location.hash==="#/")&&aa(c.page);window.addEventListener("hashchange",()=>{const a=ta(location.hash);a&&a!==c.page&&na(a,{writeHash:!1})});window.addEventListener("popstate",()=>{const a=ta(location.hash);!a||a===c.page||na(a,{writeHash:!1})});Lt();
